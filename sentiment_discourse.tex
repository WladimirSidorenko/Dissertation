...
optimal model parameters $\mathbf{\theta^*}$ that maximize the
...
\begin{equation*}
  \theta^* = \argmax_{\theta}\ell(\theta) = \sum_{i=1}^{N}\log{}p\left(\mathbf{y}^{(i)}\vert\mathbf{x}^{(i)}; \theta\right),\label{dasa:eq:crf-objective}
\end{equation*}
where conditional likelihood is normally estimated as:
\begin{equation*}
  p\left(\mathbf{y}^{(i)}\vert\mathbf{x}^{(i)}; \theta\right) =
  \frac{\exp\left(\sum_{t=1}^{T_i}\sum_k\mathbf{\theta}_kf_k\left(\mathbf{x}^{(i)}_t,\mathbf{y}^{(i)}_{t-t},\mathbf{y}^{(i)}_{t}\right)\right)}{Z}.
\end{equation*}
Adapting this equation to our RST-based structure, we obtain:
\begin{equation*}
  p\left(\mathbf{y}^{(i)}\vert\mathbf{x}^{(i)}; \theta\right) =
  \frac{\exp\left(\sum_{t=0}^{T_i}\left[%
    \sum_v\mathbf{\theta}_vf_v\left(\mathbf{x}^{(i)}_t,\mathbf{y}^{(i)}_{t}\right)
    + \sum_{c\in
      ch(t)}\sum_e\mathbf{\theta}_ef_e\left(\mathbf{y}^{(i)}_{t},
    \mathbf{y}^{(i)}_{c}\right)\right]\right)}{Z},
\end{equation*}
where $ch(t)$ denotes the children of node $t$, $v$ stands for the
indices of vertex features, and $e$ represents the indices of edge
attributes.
...
$\theta^*$ which maximize the log-likelihood of \emph{known} labels,
\ienocomma:
\begin{equation*}
\mathbf{\theta}^* =
\argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\log{}p\left(\mathbf{y}_o^{(i)}\vert\mathbf{x}^{(i)};
\theta\right).
\end{equation*}
...
maximizing the conditional log likelihood of the training set, will
now try to maximize the difference between the probabilities of
correct and wrong observed labels:
\begin{align*}
  \mathbf{\theta}^* &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\left(p(\mathbf{y}^{(i)}) - p(\mathbf{y}^{'(i)})\right)\\
  &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\exp{\mathbf{\theta}^{\top}\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)})} - \exp{\mathbf{\theta}^{\top}\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})}\\
  &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\mathbf{\theta}^{\top}\left(\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)}) - \mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})\right),
\end{align*}
...
$\mathbf{y}^{'(i)}$ respectively, and the last line follows from the
fact that both expressions (difference of the exponents and difference
of raw terms) change monotonically w.r.t. $\theta_k$, having the same
slope sign and a single critical point at
$\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)}) =
\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})$.
...
\begin{figure*}
  \centering
  {
    ...
    \begin{subfigure}{0.45\textwidth}
      \centering
      \includegraphics[width=\linewidth]{img/delta-a.png}
      \caption{$\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)}) > \mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})$}
    \end{subfigure}
  }
  {
    \centering
    \begin{subfigure}{0.45\textwidth}
      \centering
      \includegraphics[width=\linewidth]{img/delta-b.png}
      \caption{$\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)}) < \mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})$}
    \end{subfigure}
  }
  \caption{Relationship between the difference of exponents and
    difference of linear terms}\label{dasa:fig:delta-exp}
\end{figure*}

The only thing that we now need to do to the above objective is to
introduce a regularization term $\frac{1}{2}\norm{\theta}^2$ in order
to prevent the divergence to infinity in cases when
...
\begin{align}
  \mathbf{\theta}^* &=
  \argmin_{\mathbf{\theta}}\frac{1}{2}\norm{\theta}^2 -
  \sum_{i=1}^{N}\mathbf{\theta}^{\top}\left(\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)})
  - \mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})\right)
\end{align}
At this point, we can notice that the resulting objective is similar
...
until convergence is reached, whatever of these events occurs
...
by marginalizing (summing) out the remaining random terms from this
distribution, \ienocomma:
\begin{align*}
  p\left(Y=y\right) &= \sum_h p\left(Y=y, H=h\right).
\end{align*}
Applying this formula to our objective, we obtain:
\begin{align*}
...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{p{0.162\columnwidth} % first columm
      ...
      %% General Statistics:
      LMCRF & \textbf{0.76} & 0.79 & \textbf{0.77} & %
      ...
      %% General Statistics:
      LMCRF & \textbf{0.64} & \textbf{0.69} & 0.66 & %
