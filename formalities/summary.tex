...
the ways and means by how people express their opinions, evaluations,
and appraisals on this service, examines current approaches to
automatic mining of these feelings (that is what the field of
sentiment analysis is actually concerned with), and proposes novel
analysis methods that outperform state-of-the-art techniques.
...
\begin{itemize}
  \item\emph{subsentential}, in which I try to predict the polarity of
    single words and find the most important components of an opinion
    (its target, holder, and the actual evaluative statement) within a
    sentence;

  \item\emph{sentential}, where I consider the whole microblog as a
    single sentence and try to classify its overall semantic
    orientation,

  \item and, finally,\emph{suprasentential}, in which I try to improve
    on message-level sentiment classification by computing the overall
    polarity of a tweet from semantic orientations of it single
    sentences, taking into account their discourse relations (\ie{}
    semantic and pragmatic links).
\end{itemize}

\section{Research Questions}

By pursuing these goals, I am trying to find answers to the following
questions:
