...
labeled tweets\footnote{\citet{Tang:14a} assigned polarity labels to
  the messages based on the occurrence of positive and negative
  smileys in these tweets.}  in order to learn hybrid word embeddings.
In contrast to the standard word2vec vectors~\cite{Mikolov:13} and
purely task-specific representations~\cite{Collobert:11}, such
embeddings were trained to optimize both objectives---predicting the
...
aforementioned hybrid vectors, trying to predict the noisily obtained
labels, and applied this classifier to each learned embedding in
isolation, considering the predicted score as the polarity value of
the respective word or phrase.  Similarly, \citet{Vo:16} also adopted
the distant supervision framework.  However, in contrast to the former
...
seeds appeared closest and most frequently in its neighborhood.
...
furthest away from each other.  Considering the inverse distances to
the origins of these axes as subjectivity and polarity scores of the
projected words respectively, we considered the sum of these values as
...
polarty scores of the remaining terms with unknown semantic
orientations.

In particular, we first determined the subjectivity axis
$\vec{b}_{subj}$ which maximized the mutual distance between the sets
of subjective and objective words
($\mathcal{S} = \mathcal{P} \cup \mathcal{N}$ and $\mathcal{O}$
respectively), and then computed the subjectivity values for polar and
neutral terms
as~$s_{subj} = 1 + \frac{\delta_{subj}^i}{\delta_{subj}^{\max}}$ and
$s_{subj} = 1 - \frac{\delta_{subj}^i}{\delta_{subj}^{\max}}$
respectively, where $\delta_{subj}^i$ denotes the distance between the
projection of the $i$-th term and the origin $O_{subj}$, and
$\delta_{subj}^{\max}$ is the such maximum distance observed in the
data.

After obtaining subjectivity values, we similarly estimated polarity
scores by first finding a projection line~$\vec{b}_{pol}$ which
maximized the difference between the positive and negative seeds
($\mathcal{P}$ and~$\mathcal{S}$); then computing the origin of this
line~$O_{pol}$; and, finally, taking the
term~$s_{pol} = 1 + \frac{\delta^i_{pol}}{\delta^{max}_{pol}}$ as the
respective polarity value of the $i$-th term.

In the concluding step, we joined the two score types (subjectivity
and polarity) into a single score:
{\small%
  \begin{align}
    s &= \frac{1}{s_{subj} + s_{pol}},
\end{align}\normalsize}%
and sorted the resulting term list in the ascending order of these
ranks.  The pseudo-code of this method is shown in
...
the linear projection method not only outperforms every other
NWE-based algorithm in terms of the micro-averaged \F-score, but also
surpasses the results of dictionary-, corpus-based, and semi-automatic
approaches, being only 0.1~percent below the overall best \F-value
achieved by the intersection the GPC~\cite{Waltinger:10},
SWS~\cite{Remus:10}, and ZPL lexicons~\cite{Clematide:10}.  This
method also achieves the second-best macro-averaged \F-result, being
outpferformed by the $k$-NN algorithm, which, in general, establishes
a very competitive baseline mainly due to a high precision of the
recognized positive terms and a good recall of negative expressions.
Finally, the third-best micro-average is attained by the algorithm
of~\citet{Tang:14a}, which, however, suffers from a low precision of
positive entries.
...
\begin{table}[thb!]
  \begin{center}
    ...
    \begin{tabular}{p{0.15\columnwidth} % first columm
      ...
      PCA & 0.401 & 0.398 & 0.398 & \textbf{0.402}\\
...
\begin{table}[thb!]
  \begin{center}
    ...
    \begin{tabular}{p{0.15\columnwidth} % first columm
      ...
      PCA & 0.401 & 0.396 & 0.401 & 0.\textbf{405}\\
