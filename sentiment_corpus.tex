...
$\kappa$-metric: here, the agreement on sentiments has gained 30.73
percentage points, reaching $61.94\%$.  Likewise, the reliability of
opinion subjects and holders has improved by $30.42\%$ and $37.37\%$,
running up to $57.27$ and $71.12$ percent respectively.
...
coder had regarded them as valid intensifiers.  Nevertheless, even
...
Having obtained these figures, we turned to a qualitative inspection
of diverging annotation cases in our corpus.  A sample sentence with
different analyses of sentiments is shown below:
...
Additionally, we should note that it also was possible to consider the
noun phrase ``die \#afd'' (\emph{the \#afd}) as the object of the
...
label the complete verb phrase and not only the acting subject.  With
...
describing two completely different events so that labeling the same
targets, e.g., ``die Partei'' (\emph{the party}), in both of these
messages would be unequivocally wrong in this case.
...
situation is shown in the following tweets:
...
\emph{blocking politics} as the object of the evaluation, whereas the
second expert regarded the political party being accused of such
...
elements had to be labeled in this message: the one with the target
...
``Frieden'' (\emph{peace}), whereas the second expert regarded these
tokens as valid opinionated terms.  Even though our original
...
(cf.~Section~\ref{sec:snt:lex}), labeling these items turned out to be
a better solution when comparing the corpus annotation with the
existing sentiment lexica.

In addition, we should admit that these different takes of subjective
facts were to some extent due to the adjudication procedure that we
applied in step two: a closer analysis of these cases revealed that,
at the initial stage, our experts had had opposite preferences
regarding subjective facts (viz., the first annotator had typically
annotated them, whereas the second coder had usually skipped these
words).  During the revision run, however, both assistants interpreted
their decisions as false, and both changed their minds.  Even though
the rest of their changes made in that step has still lead to
significant improvements, the possibility of mutual concessions needs
to be kept in mind when applying the adjudication method in future.

In order to see whether our experts also agreed on the attributes of
the annotated items, we additionally computed the Cohen's $\kappa$ and
Krippendorff's $\alpha$ \cite{Krippendorff:07} of the polarities and
intensities of matching sentiments and emotional expressions.  The
reason for choosing two different metrics to assess the quality of the
agreement of these two elements, was that the polarity feature was a
binary attribute so that it only sufficed to estimate whether its
values were equal or not.  Intensity, on the other hand, was a
categorial trait with its values were ranging from weak (0) to strong
(2) and, therefore, disagreements which were far apart had to be
penalized more than divergencies that were nearby.

For estimating the latter term, we used the ordinal distance measure,
interpreting weak, medium, and strong intensities as zero, one, and
two respectively.  The results of these computations are shown in
