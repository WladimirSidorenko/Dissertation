...
a set of manually annotated entities $\mathcal{S}$ and automatically
tagged spans $\widehat{\mathcal{S}}$, \citeauthor{Johansson:10a}
estimate precision of automatic assignment as:
...
automatically ($s_j$) annotated entities:
\begin{equation*}
...
Similarly, recall of this assignment is estimated as:
\begin{equation*}
  R(\mathcal{S}, \widehat{\mathcal{S}}) = \frac{C(\mathcal{S},
    \widehat{\mathcal{S}})}{|\mathcal{S}|};
\end{equation*}

Using these two values, we can normally compute the \F{}-measure as:
\begin{equation*}
...
\item\emph{morphological}, which encompassed part-of-speech tags of
  ...
  types; degree of comparison for adjectives; and mood, tense, and
  person forms of verbs;
...
the authors also use activation and forget gates ($\vec{i}^{(t)}$ and
...
therefore be optimized with the usual gradient update techniques.
...
of the trained model (if the steps are too large).
