...
Each word is associated to a vec- tor representation, which consists
...
Preprocessing and word embeddings.  The word embeddings are
...
Before extracting features, the tweets were preprocessed using the
...
proposed by~\citet{Rouvier:16}.  In contrast to the former systems
which utilized sentiment-flavored pre-trained word vectors with a
single convolutional layer, the authors simultaneously harnesses three
...
final step, \citeauthor{Rouvier:16} joined the outputs of the deep
convolutions and perceptron into a single vector, subsequently passing
this vector to two fully connected neural network layers with softmax
non-linearity.  This way, the authors achieved 0.63~\F{} on the
