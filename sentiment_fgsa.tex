...
\item\emph{formal}, which included the initial three characters of
  ...
  }\mathbf{x}_m\sim\mbox{ /\textasciicircum abc/ else } 0$), its last three
  characters, and the general spelling class (e.g., alphanumeric,
  digit, or punctuation);

\item\emph{morphological}, which encompassed the PoS tag of the
  analyzed token, case and gender values for inflectable PoS types,
  degree of comparison for adjectives; and mood, tense, and person
  forms for verbs;
...
\item and, finally, \emph{syntactic}, which included the dependency
  ...
  parent, the lemma of the parent node, the PoS tag and the polarity
  class of the grandparent, as well as the overall polarity of all
  children.  The latter term was computed by summing up polarity
  scores of all immediate descendants and checking whether the
  resulting value was greater, less, or equal to zero.
\end{itemize}
...
\begin{itemize}
\item the dependency relation of the previous token in the sentence to
  its parent + dependency relation of the current token to its
  ancestor;
\item the dependency relation of the next word in the sentence +
  dependency relation of the current token;
\item the lemma of the child node + dependency relation of this child;
...
Table~\ref{snt-fgsa:tbl:crf-res}.
...
\item the ability of these systems to learn the optimal feature
  ...
  traditional supervised machine learning frameworks, where all
  features need to be defined by user; and
\item the ability to deal with arbitrary sequence lengths, which
  advantageously distinguishes them from other NN architectures, where
  the size of the learned parameters needs to be fixed.
\end{inparaenum}

The key component which is common to any modern RNN system is a
...
of the input seen so far.  The general form of this vector at time
...
previous time step, and $\mathbf{x}^{(t)}$ is the input at step $t$.

A fundamental problem which arises from the above definition is that
the gradients of the trained parameters, which are used to estimate
the $\vec{h}$ vector, rapidly vanish to zero or explode to infinity
(depending on whether the abosule values of $\vec{h}$ are less or
greater than one) as the length of the input increases.  In order to
solve this issue, \citet{Hochreiter:97} proposed the long short-term
memory mechanism (LSTM), in which they explicitly incorporated the
goal to drop parts of the input which seemed to be irrelevant for the
final outcome.  In particular, given an input sequence $\mathbf{x}$,
they introduced an intermediate input unit $\vec{i}^{(t)}$:
\begin{align*}
  \vec{i}^{(t)} &= \sigma\left(W_i\cdot \mathbf{x}^{(t)} + U_i \cdot \vec{h}^{(t-1)} + \vec{b}_i\right),
\end{align*}
where $\sigma$ is the sigmoid function; $W_i$, $U_i$, and $\vec{b_i}$
denote the optimized model's parameters; and $\mathbf{x}^{(t)}$ and
$\vec{h}^{(t-1)}$ are defined as previously.  From this intermediate
representation, they then computed the candidate value $\widetilde{C}^{(t)}$
for the current time step:
\begin{align*}
  \widetilde{C}^{(t)} &= tanh\left(W_c\cdot \mathbf{x}^{(t)} + U_c
  \cdot \vec{h}^{(t-1)} + \vec{b}_c\right).
\end{align*}
After computing the forget unit~$f^{(t)}$ as:
\begin{align*}
  \vec{f}^{(t)} &= \sigma\left(W_f\cdot \mathbf{x}^{(t)} + U_f
  \cdot \vec{h}^{(t-1)} + \vec{b}_f\right),
\end{align*}
they finally estimated the activation candidate for time step $t$ as a
weighted sum of the current candidate and the previous activation
state:
\begin{align*}
  C^{(t)} &= \vec{i}^{(t)} * \widetilde{C}^{(t)} + \vec{f}^{(t)} * C^{(t-1)}.
\end{align*}
From this activation, they then computed the output vector
...
An alternative recurrent architecture, which also mitigated the
problem of vanishing and exploding gradients, was proposed
by~\citet{Cho:14a}.  In their work, the authors also used activation
and forget gates similar to the ones defined by~\citet{Hochreiter:97},
though without the gate-specific bias terms ($\vec{b}_i$ and
$\vec{b}_f$ respectively):
\begin{align*}
...
\begin{align*}
  ...
  \cdot \left(\vec{f}^{(t)}\odot\vec{h}^{(t-1)}\right)\right);
\end{align*}
