...
any object (e.g., a product, service, or political decision), FGSA is
commonly regarded to be one of the most attractive, necessary, but,
...
assign their labels, and the evaluation metrics, using which we will
...
decisions of the coders, we explicitly provided a rule for determining
opinions' boundaries by telling our experts to assign the
...
metric only regards as true positive those automatic spans which have
absolutely identical boundaries with the gold assignment.
Unfortunately, both of these approaches are somewhat problematic:
While the binary overlap might be overly optimistic, always assigning
perfect scores to automatic spans which cover the whole sentence; the
exact match might, vice versa, be too drastic, considering the whole
...
\end{equation*}
and $|\widehat{\mathcal{S}}|$ denotes the total number of spans
automatically labeled with the given tag.  Similarly to precision, the
recall of such assignment was estimated as:
\begin{equation*}
...
\subsection{Data Preparation}\label{snt:fgsa:subsec:data}
...
which was dominating the NLP field for more than a decade.  The main
resons for this huge success are:
\begin{enumerate}[1)]
...
\begin{example}[Overlapping and Correlated Features]
  ...
  distribution in our corpus.  However, if we replaced ``Merkel'' with
  ``von der Leyen'' both in the training data and test example and
  rerun this experiment, the probability would get significantly
  skewed towards the CDU class:
  \begin{align*}
  ...
  This probability is usually computed using the sigmoid function
...
\item and, finally, the \emph{avoidance of the label bias problem}, to
  ...
  Markov networks \cite{McCallum:00}, are known to be susceptible.
  \begin{example}[Label Bias Problem]
    ...
    be preferred during the automatic tagging.
    ...
    probability of the correct labels runs up to:
    \begin{align*}
...
input variables of the $n$-th training instance and $\mathbf{y}^{(n)}$
denotes their gold labels, CRF's training adds up to finding feature
coefficients $\mathbf{w}$ that maximize the log-probabilities $\ell$
of $\mathbf{y}^{(i)}$ given their respective covariates
...
Equation~\ref{snt:fgsa:eq:crf-ell} analytically with a single shot.
One therefore has to resort to other optimization techniques such as
the gradient descent method, in which the weights of the features are
successively changed towards the direction of the gradient until the
global minimum of the loss function is reached.

We can easily see that the partial derivative of the log-likelihood
from Equation~\ref{snt:fgsa:eq:crf-ell} w.r.t. the single feature
weight $w_j$ amounts to the following solution:
\begin{equation}
...
the model's parameters $\mathbf{w}$.  In other words, the optimal
solution to the log-likelihood function in
...
computing the model's expectation, can be estimated dynamically using
the forward-backward (FB) algorithm \cite{Rabiner:90}---a particular
instance of the more general belief propagation method
...
rank, and used He initialization~\cite{He:15} for setting the values
of the bias vectors.
...
predicting sentiments as their removal unequivocally leads to some
degradation of the scores.  This quality drop, however, is usually
quite small, suggesting that other attributes types can easily make up
...
\subsubsection{Effect of Word Embeddings}

\subsubsection{Effect of Lexica and Normalization}

\subsubsection{Effect of Topologies}
