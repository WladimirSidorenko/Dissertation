...
the proportional $A_1$ scores of these two entity types come to 8,243
and 3,326 tokens, whereas the corresponding $A_2$ figures amount up to
...
cases revealed the fact that the majority of these disagreements
...
the second coder had regarded them as valid intensifier cases.
Nevertheless, even despite these diverging interpretations, the
reliability on intensifying elements was above the $55\%$ threshold,
reaching a moderate agreement level.
...
With these figures in mind, we turned to a qualitative inspection of
diverging cases.  In doing so, we first looked at disagreeing
labelings of our experts, trying to understand the reasons for their
different interpretations, and then analyzed the agreement on the
attributes of the annotated items in order to see whether our
assistants could similarly perceive the properties of the elements
they were labeling.

Below, we provide a couple of examples with contradicting analyses
taken from our final corpus.  A sample sentence with diverging takes
of sentiments is shown in the following message:
...
\end{example}
As can be seen from this analysis, the first coder erroneously
...
Additionally, we should note that it also was possible to consider
only the noun phrase ``die \#afd'' (\emph{the \#afd}) as the object of
the evaluation in this message.  We, however, advised the assistants
to be as specific as possible when determining the targets of the
...
label the complete verb phrase and not only the acting subject of that
clause.  With this rule, we hoped to better distinguish targets in
sentences like ``die Partei das Gesetz verabschiedet \smiley{}''
(\emph{the party has adopted this law \smiley{}}) from the objects of
evaluations in microblogs like ``die Partei das Gesetz abgelehnt
\smiley{}'' (\emph{the party has rejected this law \smiley{}}), which
were clearly describing two completely different events so that
labeling the same target span, e.g., ``die Partei'' (\emph{the
  party}), in both of these messages would be unequivocally wrong in
that case.
...
situation is shown in the following message:
...
\emph{blocking politics} as the evaluated object, whereas the second
expert regarded the political party being accused of such behavior as
the target of the sentiment.  In our opinion, both of these options
were valid in this case and, ideally, two sentiments had to be labeled
in this message: the one with the target ``Blockadehaltung''
(\emph{blocking politics}) and another one with the target ``die SPD''
(\emph{the SPD}).

Even though our assistants were much more consistent when labeling
emotional expressions than when analyzing opinion-level elements, we
decided to have a look at diverging annotations of these entities too,
in order to understand the reasons for the remaining conflicts.  A
...
primary source of annotators' confusion: the first coder skipped the
words ``Angriff'' (\emph{attack}), ``Bombe'' (\emph{bomb}), and
``Frieden'' (\emph{peace}), whereas the second expert regarded them as
valid opinionated terms.  Even though our original guidelines were
underspecified in this case (we only imposed the subjectivity
constraint on sentiments), in our later experiments
(cf.~Section~\ref{sec:snt:lex}), labeling such items turned out to be
a better solution when comparing the corpus annotation with the
existing sentiment lexica.

In addition, we should say that these different takes of subjective
facts were also partially due to the adjudication procedure that we
applied: a closer analysis of these cases revealed that, at the
initial stage, our experts had had opposite preferences regarding the
non-evaluative polar terms (viz., the first annotator had typically
annotated them, whereas the second coder had usually skipped these
words).  During the second annotations stage, both assistants
interpreted their decisions as false, and both changed their minds.
Even though the rest of their changes made in that step has still lead
to significant improvements, the possibility of mutual concessions
needs to be kept in mind when applying the adjudication method in
future.
