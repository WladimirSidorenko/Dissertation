...
from the NLP community.  One of the first reported attempts to mine
users' evaluations automatically on CMC services was made by
...
up) and \emph{not recommended} (thumbs down) ones.  These decisions
were made by summing the pointwise mutual information (PMI) scores
between the adjectival and adverbial phrases occurring in the review
text and the word ``excellent'' and then subtracting from this sum the
PMIs of these phrases in conjunction with the word ``poor''.

\citet{Turney:02} noted in their experiments that the movie critiques
were by far the most difficult type of reviews to analyze (the
...
also confirmed by \citet{Pang:02} who tested several machine learning
classifiers on a manually annotated corpus of $\approx2,000$ Internet
Movie Database (IMDb) write-ups.  The best results (82.9\% accuracy)
for this task were attained by the SVM system which used the presence
of unigrams as its classification features.
