...
\item
  ...
  potential subjective statement had to satisfy in order to be marked
...
\item
  ...
  \begin{itemize}
    \item
      ...
      comparative opinions to distinguish between the favored objects
      and the rest of the entities being compared;
    \item
      the link attribute \emph{anaphref}, which was used for targets
      expressed by pronouns and had to point to the respective
...
their dependent grammatical and text-level adjuncts and complements.
...
dependent on the main evaluated term.
...
\item
  ...
  the whole opinions (i.e., contextual polarities and intensitis of
  evaluations).
...
idioms, ignoring their possible syntactic attributes.
...
intensifier, and negation, on the other hand, only comprise single
words without their syntactic attributes.  The polarity of the
opinionated term is \emph{positive}, since this is the primarily
expressed attitude of this term outside of the context.  Both the
intensity of the emotional expression and the degree of its
intensifier ``sehr'' (\emph{very}) are set to \emph{medium}, and the
link attribute \emph{emo-expression-ref} unequivocally points to the
element being modified by the negation and intensifying
item.\footnote{A more detailed description of the annotation elements
  and their possible attributes along with the examples of ambiguous
  cases can be found in the original annotation guidelines provided in
...
scheme and customization settings, which specified what kinds of
elements with which attributes were to be annotated by the human
coders, and how these elements had to look like.
...
level structure of our project consists of three main directories: {
  ...
  \item and, finally, \texttt{scripts/}, which comprises auxiliary
    scripts for estimating the inter-annotator agreement and aligning
    corpus annotations with automatically parsed corpus sentences.
  \end{itemize}
}
