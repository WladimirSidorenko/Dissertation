...
One of the first work on automatic sentiment classification with
ML-based methods was done by~\citet{Wiebe:99}, who trained a Na{\"i}ve
Bayes system to differentiate between subjective and objective
statements, using binary features reflecting the presence of a
pronoun, an adjective, a cardinal number, or a modal other than
``will'' in the sentence as input.  The author reached an accuracy
of~72.17\%, outperforming the majority class baseline by more than
20~percentage points.  An even better result (81.5\%) could be
achieved when the data set was restricted only to the examples with
the most confident annotation.
