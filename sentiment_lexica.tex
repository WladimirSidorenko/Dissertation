...
With the first approach, we used the embeddings of the seed terms
$\mathcal{P} = \{p^+_1,\ldots,p^+_k\}$, $\mathcal{N} =
\{p^-_1,\ldots,p^-_m\}$, and $\mathcal{O} = \{p^0_1,\ldots,p^0_n\}$ to
compute the centroids of the positive, negative, and objective classes
respectively, and then assigned a new word $w$ to the nearest polarity
group whose centroid was the nearest to the $w$'s embedding.  We
considered the distance to the nearest centroid as the polarity score
of the word $w$ and sorted the resulting sentiment lexicon in the
...
A natural question which arises in this context is what this line
...
After computing the optimal projection line, we project all known word
