...
\end{align*}
they estimated the \emph{final update} state~$\widetilde{C}^{(t)}$ by
taking a weighted sum of the candidate update
vector~$\widetilde{C}^{(t)}$ and the previous update
value~$C^{(t-1)}$:
\begin{align*}
...
LSTM has often been criticized for an unduly complexity of the
...
(GRU).  In this framework, the authors also used an activation and
forget gate similar to the ones defined by~\citet{Hochreiter:97}.
They dropped however the unit-specific bias terms $\vec{b}_i$ and
$\vec{b}_f$ for simplicity:
\begin{align*}
