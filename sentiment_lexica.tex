...
works introduced by~\citet{Tang:14a}, who trained sentiment-specific
embeddings, considering the prediction of distantly supervised
sentiment labels as an additional training objective, and applied
another classifier in order to predict the polarity of these
embeddings.  Another NWE-based method was proposed by~\citet{Vo:16},
who trained two-dimensional embeddings, trying to predict the
occurrence of positive and negative emoticons in a tweet.
...
with four alternative machenisms of obtaining polarity lists from
distributed vector representations of words:
\begin{enumerate}
  \item the method of the nearest centroids,
  \item the $k$-means clustering,
...
clusters respectively.  Afterwards, we assigned a new word $w$ to the
polarity group whose centroid was the nearest to the $w$'s vector.  We
...
embedding matrix $E$ into its SVD components:
\begin{align*}
  E = U \Sigma V^T,
\end{align*}
...
of the embeddings for seed terms with opposite polarities on this axis
were maximally far apart from each other.

Since SVD, however, was not guaranteed to find the optimum polarity
scale (the orthognal bases of SVD are computed using the covariance
matrix of $E$ and therefore might not capture the semantic orientation
if terms with opposite polarities occur in the same contexts), we
additionally introduced our own \emph{linear projection method}, in
whoch we explicitly defined this objective: Namely, given the sets of
word vectors for positive and negative polarity terms $\mathcal{P} =
\{\vec{p}_{+_1},\ldots,\vec{p}_{+_m}\}$ and $\mathcal{N} =
\{\vec{p}_{-_1},\ldots,\vec{p}_{-_n}\}$ respectively, find a
projection line $\vec{b}$ which would maximize the distance between
the projections of terms with opposite semantic orientations, i.e.: {\small%
\begin{align}
  \small
...
iteration step: $\gamma = \Delta \cdot \vec{b^i}$.\footnote{A detailed
  ...
  this paper.} With this gradient, we optimized the line $\vec{b}$
using gradient ascent until we reached the global maximum of the
function $f$.
