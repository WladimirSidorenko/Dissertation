...
thoughts makes social media the arguably most suitable channel for
sharing own emotions.
...
\emph{buy} (positive attitudes), \emph{sell} (negative attitudes), and
\emph{null} (neutral messages).  With these classifiers, they
...
non-negative total scores were accordingly classified as \emph{thumbs
  up}.
...
\emph{tired}, \emph{happy} etc.).  The gold label corpus of
$\approx$~815K blog posts was obtained automatically from the
Livejournal website.  On this corpus, the author trained a supervised
SVM classifier, reaching an average of 8\% improvement over the 50\%
baseline.
