...
data.  We will summarize and conclude in the last part of this
...
lexica for German on the updated corpus data.  The most prominent of
these resources are:
\begin{itemize}
...
not only shows the highest $F1$-score for negative terms but also
achieves the best macro-averaged $F1$-result on all classes.
...
skewness of distribution could significantly affect the net outcome,
we additionally recomputed all results on the whole corpus.  The
updated figures are shown in Table~\ref{snt-lex:tbl:gsl-res-full}.
...
material and the persistent up-to-date state of these data, which can
hugely boost the recall.
...
\cite{Miller:95}.  A similar procedure was applied to antonymous
relations with the polarity orientation being reversed during the
propagation.  This expansion continued until no more adjective could
be reached via the synonymy-antonymy links.  Unfortunately, no
intrinsic evaluation of the resulting lexicon was performed in this
work---the authors only report their results on recognizing subjective
sentences and classifying their polarity, where they attain average
\F-scores of 0.667 and 0.842 respectively.
...
vector $v$ containing the polarity values of the known seed terms (-1
for the negative expressions and 1 for the positive terms) with the
adjacency matrix $A$ constructed for the \textsc{WordNet} synsets.  In
these experiments, the value of the adjacency cell $a_{ij}$ was set to
$0.2$ if there was a synonymy link between the synsets $i$ and $j$ and
to $-0.2$ if these synsets were antonymous to each other.  Performing
this multiplication multiple times and updating the $v$ vector
correspondingly after each iteration, the authors ensured that the
polarity scores got propagated transitively through the network,
decaying by a constant factor ($0.2$) with the increasing path length
from the original seeds.  This method again was evaluated only
...
\citet{Awadallah:10}, who adopted a random walk approach, estimating the
...
tested on a real-life corpus).  Moreover, because General Inquirer is
a generic standard-language dictionary, it is also not obvious whether
the systems that perform best on this list would be also applicable to
...
English \textsc{WordNet} \cite{Hamp:97}\footnote{Throughout our
  ...
  \textsc{GermaNet} Version 9.}--- and subsequently evaluating them on
our presented Twitter corpus.
...
first present some key statistics on both resources (shown in
Table~\ref{snt-lex:tbl:germanet-wordnet}) and visualize the lexical
graphs of these databases (demonstrated in
Figures~\ref{snt-lex:fig:germanet} and~\ref{snt-lex:fig:wordnet}).
...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{p{0.15\textwidth} % first columm
      ...
      \multirow{-2}{0.085\columnwidth}{\centering\scriptsize\bfseries{}Hy\-per.\newline{}Rels} & %
...
PoS-classes as adverbs and adjective satellites are missing completely
...
adjectives; therefore, both categories are treated the same way, being
represented through the adjectival synsets.
...
connecting different part-of-speech classes: As can be seen from the
figures, the strongest inter-PoS connections in \textsc{GermaNet} are
the pertainym links between the adjectives and nouns and the
participle edges between the adjectives and verbs.  The interlinks
between the nouns and verbs, however, are both much fewer in number
and more diverse in their nature.  This situation is different for
\textsc{WordNet} where the prevailing majority of the
inter-part-of-speech connections are \texttt{related\_to} links
(especially between the nouns, adjective satellites, verbs, and verbs
and adjectives) and \texttt{derived\_from} (especially between adverbs
and adjectives with their satellites).  The relations between the
adjectives and nouns are mixed though, featuring both
\texttt{related\_to} and \texttt{derived\_from} types.  As we should
see later, these links are crucial for breaking part-of-speech
dependencies of seed sets in the cases when all seeds belong to the
same PoS class.

While adopting the English methods to the peculiarities of the German
resource, we had to make the following changes and use the following
setup:
...
originally used \textsc{WordNet} links.  To obtain the seed sets for
this approach, we manually translated the original set of 54 polar
terms used by the authors into German and applied an automatic method
in order to obtain an initial set of neutral entries: In the original
implementation, \citet{Esuli:06c} considered as neutral all terms from
the General Inquirer lexicon \cite{Stone:66} which neither had a
postive or negative label in this resource and nor appeared among the
manually defined polar seeds.  We translated these terms using the
publicly available English-German dictionary (\url{www.dict.cc}),
taking the first suggested translation of each English word in the GI
which satisfied the above conditions.  (We also considered using all
possible German translations of the given neutral English terms, but
this significantly increased the cardinality of the neutral seed set
and consequently lowered the recall of polar items).

The results of our re-implementations are shown in
Table~\ref{snt-lex:tbl:swn-res}.
  ...
  \begin{center}
    ...
    \begin{tabular}{p{0.1\columnwidth} % first columm
      ...
      \toprule
          ...
          HL' & 4.2\stddev{1.6} & 40.2\stddev{10.2} & 7.6\stddev{2.7} & %
          4.95\stddev{1.9} &23.4\stddev{8} & 8.1\stddev{3} & %
          97.7\stddev{0.6} & 71.8\stddev{1.7} & 82.8\stddev{1.1} & %
          32.8\stddev{1.6} & 70.2\stddev{1.6}\\
    ...
    \caption{Evaluation of dictionary-based approaches.\\ {\small (HL
        -- \citet{Hu:04}, HL' -- \citet{Hu:04} with an extended set of
        synonymous relations, BG -- \citet{Blair-Goldensohn:08}, KH --
