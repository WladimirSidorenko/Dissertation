...
of the discourse relations would improve its results.  But, before we
begin with our experiments, we should first give a short introduction
to the most popular approaches to discourse analysis in the literature
and justify our choice of one of them.  Afterwards, in
...
discourse relations that might be incorporated into a discourse-aware
sentiment system), we summarize our findings in the last part of this
chapter.

\section{Discourse Analysis}\label{sec:dasa:theory}

Since the main focus of our attention in this chapter will be on
\emph{discourse analysis}, we should probably first clarify what
discourse analysis actually means and which common ways there are to
represent and analyze the discourse.

In a nutshell, discourse analysis is an area of research in
linguistics and natural language processing which explores language
phenomena beyond the sentence boundaries and tries to find and process
such phenomena automatically \cite{Stede:11}.  Although the scope of
this research can be quite large (ranging from the logical composition
of a document to the use of pronouns in a sentence), in our
experiments we will primarily concentrate on the coherence structure
of the text, \ie{} the division of the text into \emph{elementary
  discourse units} (EDUs) and the induction of (hierarchical)
coherence links between these EDUs.
...
particular its structuralism branch \cite{Saussure:90}.  However, a
...
the paragraph as the next level of the tagmemic grammar that was
composed of multiple sentences according to some predefined
compoisitonal principles.  Finally, \citet{Winter:77} conducted an
extensive study of different lexical means via which one could connect
two sentences and also proposed a taxonomy of semantic ties based on
his observations.  All relations in this taxonomy were divided into
two main groups: \textsc{Matching} and \textsc{Logical Sequence},
depending on whether they elaborated on the preceding content or
continued the natural course of narrative.

It is not surprising that the increased interest of traditional
linguistics in text-level phenomena also spurred the attention of the
broader NLP community.  Among the first who stressed the importance of
discourse analysis for automatic generation and understanding of texts
was \citet{Hobbs:79}, who argued that semantic links between sentences
were one of the most important components for building a coherent
discourse, and the lexical cohesion of this disocurse (\ie{} the use
of coreferent and overlapping lexical items) was an immediate result
of such relations.  Similarly to \citeauthor{Winter:77},
\citeauthor{Hobbs:79} also proposed a classification of inter-sentence
relations, which, however, comprised only three elements:
\textsc{Elaboration}, \textsc{Parallel}, and \textsc{Contrast}.
Although this taxonomy was definitely too small to cover the whole
...
automatic discourse analysis that appeared later.
...
relations and expanding it to a set of 23 items (by adding new types
such as \textsc{Circumstance}, \textsc{Solutionhood},
\textsc{Elaboration}, etc.), the authors also introduced the
distinction between nucleus-satellite and multinuclear ties, depending
on whether two clauses connected by such links were of different
importance to the content of the whole text (nucleus-satellite) or
represented equally significant, interchangeable elements
(multinuclear relations).  According to this distinction, they also
defined each relation as a set of constraints on their \emph{Nucleus}
(N), \emph{Satellite} (S), \emph{the N+S combination}, and \emph{the
  effect} of the whole combination on the reader (R).  An excerpt from
the original description of the \textsc{Evidence} relation as defined
by~\citet{Mann:88} is given in Example~\ref{dasa:exmp:rst-evidence}

\begin{example}[Definition of the \textsc{Evidence} Relation]\label{dasa:exmp:rst-evidence}
\textbf{Relation Name:} \textsc{Evidence}

\textbf{Constraints on N:} R might not believe N to a degree satusfactory to W

\textbf{Constraints on S:} The reader believes S or will find it credible

\textbf{Constraints on the N+S Combination:} R's comprehending S increases R's belief of N
...
The most important requirement imposed by the authors, however,
related to the overall structure of the text, which had to be
completely covered by such relations with their EDU arguments
essentially forming a tree.  EDUs located near the top of this tree
were supposed to play the central role to the main idea of the whole
text, with their satellite dependents providing rather auxiliary
additional information.  You can see a sample fragment of such tree
from the RST Discourse Treebank \cite{Carlson:01a} in
Figure~\ref{dasa:fig:rst-tree}.  The top-most node in this example
...
additional information on it such as commentary of another analyst.
...
\todo[inline]{PDTB \citet{Prasad:04}}
