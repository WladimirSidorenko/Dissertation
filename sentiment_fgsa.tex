...
\item and, finally, the \emph{avoidance of the label bias problem}, to
  ...
  \begin{example}[Label Bias Problem]
    ...
    \begin{center}
      ...
      \path [-] (E) edge node[below left=0.4] {$.9$} (H);
      \path [-] (F) edge node[above left=0.4] {$.5$} (I);
      \path [-] (F) edge node[above] {$.5$} (H);

      \path [-] (I) edge node[below] {$.8$} (K);
      \path [-] (H) edge node[above] {$.2$} (K);
    \end{tikzpicture}
    ...
    \begin{align*}
      ...
      \times&\frac{\exp(0.5 + 0.5)}{\exp(0.5 + 0.5) + \exp(0.5 + 0.5)}\times\frac{\exp(0.2 + 1.)}{\exp(0.2 + 1.)} = 0.25
    \end{align*}
    At the same time, the probability of the incorrect variant
    ($KON-ADJA-NN-VA$) would amount to 0.45 and will therefore be
    preferred during the automatic tagging.

    A different situation is observed with the CRFs, where the
    normalizing factor in the denominator is computed for the whole
    sequence at once without factorizing into individual terms for
    each transition as it was done in the previous case.  That way, we
    the probability of the correct labels:
    \begin{align*}
      P(KON, ADV, NN, VA) =& P(KON)\times
      P(ADV|KON)\times P(NN|ADV)\times P(VA|NN)\\ =&\frac{\exp(1 + 0.5
        \times 4 + 0.2 + 1)}{Z} = 0.18066\\
    \end{align*}
    where $Z = \exp(1 + 0.5 \times 4 + 0.2 + 1) + \exp(1 + 0.5 \times
    4 + 0.8 + 1) + \exp(1 + 0.5 \times 3 + 0.9 + 0.2 + 1) + \exp(1 +
    0.5 \times 3 + 0.1 + 0.8 + 1)$ is the enumeration of all possible
    label assignments with their respective scores.
  \end{example}
\end{enumerate}
