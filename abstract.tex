...
communicated via these platforms.  As a consequence, dealing with this
abundance of texts in manual way becomes both cumbersome and inefficient.  But
the existing natural language processing (NLP) applications do not scale up
well to the relatively peculiar genre of the online communication either.
Therefore, we urgently need more robust and more high-quality NLP programs
that are able to handle even such ``unconventional'' linguistic variations as
the Web language.  But in order to build such programs, we first need to
understand the nature of these variations better.
...
of the tweets, how frequent these phenomena are, and what role they play in
the online communication.  From the computational perspective, we evaluate to
what extent differences in the language style can affect the performance of
the existing NLP approaches and how this performance could be improved for
social media texts.
...
annotate.  After this, we demonstrate how well the sentiment classification
...
These refinements primarily concerned the definition of elementary discourse
units (EDUs) for such Twitter-specific elements like @-mentions, hyperlinks,
hashtags etc.  Furthermore, several additions to the discourse relations set
were needed in order to cover the inter-tweet relationships.  After defining
these extensions, we then evaluate methods and strategies for automatically
detecting EDU boundaries and succeeding discourse parsing on Twitter.
