...
\item and, finally, the \emph{avoidance of the label bias problem},
  ...
  \begin{example}[Label Bias Problem]
    ...
    be preferred by an automatic tagger.
    ...
    transition as it is done in the MEMM case.  This way, the
...
attributes which are defined by the developer.
...
Furtermore, a closer look at each of the settings of our experiments
revealed that:
\begin{itemize}
\item Higher sentiment scores can be achieved by narrowing the spans
  of this class to the emotional expression only.  This, however,
  might negatively affect the classification of other opinion entities
  (sources and targets);
\item Even though the context seems to play an important role in the
  ...
  however, still improve the results of the traditional linear-chain
  CRFs with their first- and second-order semi-Markov variants;
\item In order to understand why the effect of topology changes was so
  ...
  attributes massively predominated the final decisions, surpressing
...
\item Nevertheless, a closer look at the top-scoring state and
  ...
  the state attributes revealed that different types of these features
  had varying effects on different entity classes: while sentiment
  ...
  influenced by morphological and syntactic attributes only;
\item Similarly to the review of CRF features, we also analyzed the
  effect of different embedding types on the net scores of the RNN
  approaches, finding that task-specific word vectors with
  least-squares fallback to word2vec embeddings yielded best overall
  results;
...
\item Finally, in the concluding part of the evaluation, we estimated
  ...
  the best-performing German sentiment lexicons; finding that the
  lexicon features only marginally improved the CRF results, and had
  an even negative effect on the scores achieved by the LSTM and GRU
  approaches.
\end{itemize}
...
Twitter domain to the automatic extraction of fine-grained opinions.
Even though we did our best to make the foregoing description and
evaluation as comprehensive as possible, there still remain many
potential avenues to explore and potential ways to improve the
presented results: It would, for instance, be extremely helpful to
evaluate other polarity lists (produced by different SLG methods) in
combination with the described FGSA methods.\footnote{Unfortunately,
  we were not able to do this analysis by ourselves due to
  prohibitively long training times: a single run of an RNN classifier
  was taking more than 27 hours on a very strong machine.}  Another
interesting opportunity would be a modification of the CRF formalism
in which the influence from the context would be equal or reciprocal
to the sum of the state feature weights.  Finally, an ensemble of CRF
and RNN systems would be a viable possibility for improving the scores
of each of these systems.  We encourage interested readers to explore
these ways in their future research, as we soon have to turn our
attention to another big area of opinion mining in which the polarity
lists might reveal their true power: the coarse-grained sentiment
analysis.
