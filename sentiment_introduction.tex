...
Among the first who applied \citeauthor{Banfield:82}'s theory to the
...
SVM classifiers.  The best $F$-score for this two-class classification
...
negative messages.  At roughly the same time, \citet{Barbosa:10} also
gathered a collection of 200,000 microblogs from three publicly
available sentiment web-services.  Using the majority prediction of
these three services as their gold labels, the authors trained an SVM
classifier on this corpus and then let their system predict the
subjectivity and polarity classes of new unseen tweets.

In the same vein, \citet{Kouloumpis:11} trained a supervised AdaBoost
classifier \cite{Schapire:00} on a subset of tweets from the Edinburgh
Twitter corpus\footnote{\url{http://demeter.inf.ed.ac.uk}} and the
emoticon dataset\footnote{\url{http://twittersentiment.appspot.com}}
of \citet{Go:09}.  Reaching an average $F1$-measure of 68\% on a
manually labeled dataset, the authors found the microblog-specific
features (such as the presence of intensifiers or the occurrence of
abbreviations and emoticons) the most relibale traits for making
predictions.  Other important works in this direction include those of
\citet{Davidov:10}, \citet{Jansen:09}, and \citet{Agarwal:11}.  TODO:
elaborate on these works.
...
and a pre-defined emotions set.  This study revealed that Twitter
could serve as a platform for political deliberation and that the mere
number of tweets referencing some party strongly corresponded with the
results of political election polls.

As in the previous cases, a key factor for the success and popularity
of this subfield was the creation and dissemination of high-quality
big annotated corpora.  An important role in this respect was played
by the organizers of the SemEval competition on sentiment analysis in
Twitter \cite{Nakov:13,Rosenthal:14,Rosenthal:15}.
...
labeled on the crowdsourcing platform Amazon Mechanical Turk.  To this
...
participating teams, incited the organizers to continue this
competition in the following years.  With slight modifications
(addition of new tweets, sarcastic microblogs, and LiveJournal
sentences), both subtasks were offered in \citeyear{Rosenthal:14}
\cite[cf.][]{Rosenthal:14}.  Again, the system proposed by
\citet{Mohammad:13} with some minor adjustments \cite{Zhu:14} ranked
first in the phrase-polarity task and got fourth place on predicting
the overall polarity of the messages.

The top-scoring system for the second track was proposed by
...
\begin{inparaenum}[\itshape a\upshape)]
  \item message polarity classification on specific topics,
  \item topic-based trend prediction, and
  \item determining prior polarity of lexical terms.
\end{inparaenum}
