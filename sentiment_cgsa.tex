...
these datasets (23\% for SB10k and 14\% for the snapshot), but its
influence this time is much less pronounced than in the PotTS case.
As mentioned above, negative tweets are the least represented semantic
representation across all three sources.  The only group which has
even less instances than this class is the mixed polarity.  We,
...
Finally, the results of the inter-rater reliability check confirm the
superior quality of the PotTS corpus, which, even despite an
approximate inference of its labels, still has an $\alpha$
coefficient~\cite{Krippendorff:07} that is almost 1.7 times as high as
...
between distinct labels, which, for example, will be bigger for the
pair positive vs. negative than for the pair positive vs. neutral), we
find the Krippendorff's metric more appropriate for assessing the
quality of the corpora for this task.
