...
towards particular entities in discourse is called \emph{sentiment
  analysis} (SA) \citep{Liu:12}.  The definition of this field,
however, much like the definition of the term \emph{sentiment} itself
is neither complete nor universally accepted.  The reasons for these
controversies are first of all
\begin{inparaenum}[\itshape a)\upshape]
  \item a frequently unclear delimitation of the subjective and
    objective components of information (a prominent example of such
    borderline cases are the so-called subjective facts like
    \emph{bomb attack} or \emph{medical cancer treatment}) and
  \item the heteroginity of the language system, to which SA methods
    are applied.
\end{inparaenum}

Speaking of the last factor, one usually differentiates between
different types of sentiment analysis depending on the language level
that is being analyzed.  Typical subsentential, sentential, and
