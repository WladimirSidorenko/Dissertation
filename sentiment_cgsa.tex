...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{p{0.162\columnwidth} % first columm
        *{4}{>{\centering\arraybackslash}p{0.2\columnwidth}}} % last two columns
      \toprule
      \textbf{Dataset} & \multicolumn{3}{c}{\bfseries Polarity Class} & \textbf{Omitted}\\
                       & \textbf{Positive} & \textbf{Negative} & \textbf{Neutral} & \\\midrule

      \textbf{PotTS} & 3,394 & 1,541 & 2,901 & 156 \\
      \textbf{SB10k} &  &  &  &  \\
      \textbf{GTS} &  &  &  & \\\bottomrule
\end{tabular}
    \egroup
    \caption{Polarity class distribution in the PotTS corpus and
      German Twitter Snapshot (GTS).}
...
(negative) sentiments and skipping all microblogs that comprised
opinions with opposite semantic orientations.  In cases when a
sentiment was absent, we applied a fallback strategy, considering all
tweets with only positive (negative) polar terms as positive
(negative), disregarding posts that simultaneously featured
expressions from both classes, and taking the rest of the corpus
(i.e., messages with neither sentiments nor polar terms) as neutral
instances.

Finally, to derive these classes
for the snapshot posts, we followed the procedure proposed
by~\citet{Read:05} and~\citet{Go:09}, and assigned the positive
(negative) labels to the tweets which contained the respective
emoticons.
...
The first group of approaches which we are going to explore in this
...
overall polarity of complete texts appeared on the scene.
...
labeled Epinions comments.  In the same vein, \citet{Hi:04} computed
...
approaches, arguing that, besides considering the lexical valence
(i.e., semantic orientation) of polar terms, it was also necessary to
incorporate syntactic, discourse-level, and extra-linguistic factors
such as negations, intensifiers, modal operators (e.g., \emph{could}
or \emph{might}), presuppositional items (e.g., \emph{barely} or
...
\citet{Kennedy:06}, who investigated two ways to determine the
polarity of a customer review: In the first of these methods, they
simply compared the numbers of positive and negative expressions
appearing in text, assigning the review to the class with the greater
number of items.  In the second attempt, they enhanced the original
system with additional information about contextual valence shifters,
