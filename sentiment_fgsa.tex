...
overfitting effect, due to which, attributes which actually give rise
to subjective evaluations are often disregarded in favor of less
indicative features.  Since the latter attributes, might not to appear
in the test data or are unlikely to correlate with the sentiment
entities even if they occur, the model often fails to recognize
sentiments in contexts which do have indicative but underweighted
features.
...
An alternative recurrent architecture (Gated Recurrent Units or GRU),
which also mitigated the problem of vanishing and exploding gradients,
was proposed by~\citet{Cho:14a}.  In their work, the authors also used
activation and forget gates similar to the ones defined
by~\citet{Hochreiter:97}, though without the gate-specific bias terms
($\vec{b}_i$ and $\vec{b}_f$ respectively):
\begin{align*}
