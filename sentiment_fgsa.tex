...
prohibitively expensive to find the best analytical solution to
Equation~\ref{snt:fgsa:eq:crf-ell} with a single shot.  One therefore
has to resort to other optimization techniques such as the gradient
descent method, in which the weights of the features are successively
changed towards the direction of their gradient until the global
minimum of the loss function is reached.

We can easily see that the partial derivative of the log-likelihood
function from Equation~\ref{snt:fgsa:eq:crf-ell} w.r.t. the single
