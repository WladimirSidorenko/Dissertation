...
\todo[inline]{\citet{Pang:05}}

\done[inline]{\citet{Bickerstaffe:10}}

\citet{Bickerstaffe:10} also considered the problem of multi-class
rating prediction, addressing this task with a minimum-spanning-tree
(MST) SVM approach.  In the initial step of this method, they first
constructed a strongly connected graph whose vertices were associated
with the most representative example (determined via the average
all-pairs Tanimoto coefficient) of each star rating and edge weights
represented the Tanimoto distances between these nodes.  Afterwards,
they determined the MST of this graph using the Kruskal's
algorithm~\cite[see][pp.~567--574]{Cormen:09} and, finally,
constructed a decision tree from this MST, replacing the vertices with
binary SVM classifiers, which had to discern the respective rating
groups. An evaluation on the four-star review corpus
