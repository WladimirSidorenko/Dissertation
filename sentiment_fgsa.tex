...
on the training data $\mathbf{X}$.  More precisely, given a set of
training instances $\mathcal{D} = \{(\mathbf{x}^{(n)},
\mathbf{y}^{(n)})\}_{n=1}^N$, CRF's training consists in finding
feature coefficients $\mathbf{w}$ which maximize the log-probabilities
...
\end{equation}
where $\ell(\mathbf{y}^{(n)}|\mathbf{x}^{(n)})$ denotes the
conditional log-likelihood $\ln P(\mathbf{y}^{(n)}|\mathbf{x}^{(n)})$
of the gold labels for the $n$-th training instance.  This likelihood
is commonly estimated using a globally normalized softmax function:
\begin{equation}
  P(\mathbf{y}^{(n)}|\mathbf{x}^{(n)}) = \frac{
    \exp(\sum_{m=1}^{M}\sum_jw_{j} \times f_j(x_{m}, y_{m-1},
    y_{m}))}{Z},
\end{equation}
where $M$ denotes the length of the $n$-th training instance
$|\mathbf{x}^{(n)}|$, $f_j(x_{m}, y_{m-1}, y_{m})$ stands for the
value of the $j$-th feature function $f$ at position $m$, $w_j$
...
\begin{equation}
  Z =
  \sum_{y'\in\mathcal{Y},y''\in\mathcal{Y}}\exp(\sum_{m=1}^{M}\sum_jw_{j} \times f_j(x_{m}, y'_{m-1}, y''_{m})).
\end{equation}
...
\end{equation}
After dividing both parts of the equation with the constant term $N$,
we obtain the final form for the partial derivative:
\begin{equation}
...
the model's parameters $\mathbf{w}$.  This way, at the convergence
point of the model, the expected values of its features should closely
match the expectations of these features in the observed training set.
