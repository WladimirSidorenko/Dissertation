...
ignore the structural nature of their input by either considering it
...
the discourse is composed of multiple coherent sentences.  Moreover,
apart from this inherent structural heterogeneity, even units of the
same level might play a different role and be of unequal importance
when joined syntagmatically into the higher-level whole: for example,
in words, the root morpheme typically conveys more lexical information
than the affixes; in sentences, the syntactic head usually dominates
its grammatic dependents; and, in discourse, one of the sentences
frequently expresses more prominent ideas than the rest of the text.
...
aware of discourse phenomena would improve the results of this method.
But before we proceed to our experiments, we first would like to make
a short digression and give an overview of the most popular approaches
to discourse analysis that exist in the literature nowadays.
Afterwards, in Section~\ref{sec:dasa:data}, we will describe the way
how we inferred and added discourse information to the PotTS and SB10k
data.  Then, in Section~\ref{sec:dasa:methods}, we will summarize the
current state of the art in discourse-aware sentiment analysis (DASA)
and also present our own methods, evaluating them on the
aforementioned corpora.  After analyzing the effects of various common
factors on discourse-aware sentiment analysis methods (such as the
impact of the underlying sentiment classifier and the amenability of
various discourse relation subsets), we will recap our results and
summarize our findings in the last part of this chapter.
...
composition of the entire document), in our work we will primarily
...
pieces and inferring semantic relationships between these parts is
anything but new, dating back to the very origins of general
...
previous content or adding new information to the preceding narrative.
...
could exist between two clauses, this division had laid the
...
the authors also divided all coherence links into nucleus-satellite
...
the content of the whole text.  Based on this division, they formally
...
these predefined coherence links.  You can see an example of such tree
from the original Rhetorical Structure Treebank \cite{Carlson:01a} in
...
\cite{Wolf:05} and arbitrariness of its distinguished relations
...
the Penn Discourse Treebank \cite{Prasad:04})---has been developed by
the research group of the University of Pennsylvania
...
focused on the grammatical and lexical means that can connect two
sentences (\emph{connectives}) and express a semantic relationship
(\emph{sense}) holding between these predicates.  Typical such means
are coordinating or subordinating conjunctions (\eg{} \emph{and},
...
\textsc{Contrast}, or some other sense between their two sentential
arguments (\textsc{Arg1} and \textsc{Arg2}).
...
text but can be easily inferred by the reader.  They denote these
cases as \emph{implicit} relations and demand the arguments of such
...
\end{itemize}
Finally, they also provide a special \textsc{Attribution} label for
marking the authors of reported speech.
...
the rhetorical structure theory, as it allows the discourse units
...
arguments (\eg{} \emph{in fact}, \emph{while}, or \emph{if}).  But, at
the same time, the structure of this annotation is completely flat so
that we can neither infer which of the sentences plays a more
prominent role in the text nor see the modification scope of other
...
representation explictly allows this structure to be any graph and not
only tree (\ie{} a discourse node can have multiple parents and also
have multiple links to the same vertex), provided that it does not
violate the right-frontier constraint (\ie{} has no crossing
dependencies).  In this respect, SDRT can be viewed as a
\emph{structural generalization of RST}.
...
\textsc{Contrast} link is similar to the RST \textsc{Comparison}.
...
sentiment experiments, we made our decision by considering the
following theoretical and practical concerns: From theoretical
perspective, we wanted to have a strictly hierarchical discourse
structure for each analyzed tweet so that we could infer the polarity
of that message by recursively accumulating the polarity scores of its
elementary discourse units in a bottom-up way.  From practical point
of view, since there was no discourse parser readily available for
German, we wanted to have a maximal assortment of such systems
available for English so that we could pick one that would be easiest
to retrain on German data.  Fortunately, both of these considerations
have lead us to the same solution---Rhetorical Structure Theory, which
was the only formalism that explicitly guaranteed a single root for
the analyzed text, and also offered a wide variety of open-source
parsing systems at our disposal
...
out all tweets that had at most one segment,\footnote{Since the focus
  of this chapter is mainly on discourse phenomena, we skip all
  messages which consist of a single discourse segment, because their
...
and 3,763 posts (9,625 segments) for the SB10k corpus.  Afterwards, we
assigned the polarity scores to thesw EDUs with the help of our
lexicon-based attention classifier, analyzing each elementary unit
separatey.  We again used the same 70-10-20 split into training,
development, and test sets as we did in our previous experiments.
...
Figure~\ref{dasa:fig:data-distribution}, most multi-segment messages
typically have two or three EDUs, whereas tweets with more than three
disourse units are extremely rare, which is also not surprising
regarding that the maximum length of a microblog is limited to 140
characters.  Nonetheless, even with this severe restriction, there
still are a few messages with up to 13 discourse units.  Since it was
somewhat unexpected for us to see that many segments in a single
tweet, we decided to look at these cases in more detail.  As it turned
out, such high number of EDUs typically arose from spurious
punctuation marks, which were carelessly used in texted tweets and
evidently confused our segmenter (see
...
distribution of polar classes in messages with multiple EDUs largely
corresponds to the frequencies of these polarities in the complete
datasets: For example, the positive semantic orientation still
...
microblogs again are the least represented class in both cases and
account for only 22\% of the former dataset and 16\% of the latter
tweebank.

To obtain RST trees for these segmented messages, we retrained the
DPLP discourse parser of~\citet{Ji:14} on the Potsdam Commentary
Corpus \cite[PCC~2.0; ][]{Stede:14}, after converting all discourse
relations of this dataset to the binary scheme
$\{$\textsc{Contrastive}, \textsc{Non-Contrastive}$\}$ as suggested
...
original DPLP implementation though, we did not use Brown cluster
features \cite{Brown:92}, since this resource was not available for
German, nor did we utilize the linear projection of the features,
because the released parser code did not include this component
either.  In part due to these modifications, but mostly because of the
...
0.396~\F{} for the span, nuclearity, and relation prediction on
PCC~2.0 versus 82.08, 71.13, and 61.63~\F{} on the English
corpus.\footnote{Following \citet{Ji:14}, we use the span-based
  evaluation metric of~\citet{Marcu:00}.}
...
correctly distinguish between contrastive and non-contrastive
relations, but apparently struggles with the disambiguation of the
nuclearity status, assigning the highest significance to the initial
...
multi-nuclear \textsc{Contrast} link.
...
the \textsc{Non-\-Con\-tra\-sti\-ve} coherence edges clearly dominate
...
comparison, the precentage of these links in PCC~2.0 amounts to 90\%
and also represents the prevailing majority of all coherence ties in
this corpus.
...
analysis.\todo{Redefine this objective} But, before we proceed with
this check, let us first revise the most prominent approaches to
...
unobserved labels in the input is to use the fact that we can estimate
the probability of a subset of random variables from a joint
distribution over a larger set by marginalizing (summing) out the
remaining terms of this distribution, \ienocomma:
\begin{align*}
...
the \textsc{Root}, for which we again will use the scores returned by
the LBA classifier for the whole tweet.
...
normal distribution:
\begin{align*}
  M_r \sim \mathcal{N}_{3\times3}(\boldsymbol{\mu}_r, \mathds{1}),
\end{align*}
...
\begin{equation*}
  ...
  \end{bmatrix}.
\end{equation*}
The variance of this distribution is set to the all-ones matrix.
...
node.  Thus, setting the second row to $[0., 0.3, 0.]$ will dampen the
neutral polarity of the child, which reflects our wish to decrease the
effect of neutral segments on the overall polarity of a tweet if other
strongly polar EDUs are present.  (At the same time, we still want to
assign the neutral semantic orientation to the message if all of its
RST nodes are objective.)
...
random variable drawn from the Beta distribution $B(5., 5.)$, which
...
The only thing that we now need to do to the above vector
$\boldsymbol{\alpha}_{i_j}$ before we draw the actual probability
$\boldsymbol{z}_{i_j}$ from the corresponding Dirichlet distribution
$Dir(\boldsymbol{\alpha}_{i_j})$ is to scale it by a certain
factor.\footnote{Since all elements of $\boldsymbol{\alpha}_{i_j}$
  will be in the range $[0,\ldots,1]$, the variance of the resulting
  probability will be extremely high, and the polarity scores drawn
  from this distribution will be almost random.} We compute this
...
\end{align*}
where $\xi$ is a model parameter drawn from the $\chi^2$-distribution:
$\xi\sim\chi^2(34)$; 0.1 is a constant used to prevent zero scales in
the cases when $\cos\left(\mathbf{z}^*_j, \mathbf{z}_{i_{j-1}}\right)$
is zero; and $H\left(\boldsymbol{\alpha}_{i_j}\right)$ stands for the
entropy of the $\boldsymbol{\alpha}_{i_j}$ vector.

Although this formula looks somewhat complicated, the intuition behind
it is very simple: The $\xi$ term encodes our belief in the
correctness of the model's prediction (the higher its value, the more
we trust the model); the cosine term measures the similarity between
the probabilities of parent and child (the more similar these
probabilities are, the higher will be the scale); and, finally, the
entropy in the denominator tells us how uniform the vector
$\boldsymbol{\alpha}_{i_j}$ is (the more uniform the polarity scores
are, the less confident we will be in the final outcome).
...
the $i$-th node and move on the next vertex in the reverse BFS order.
...
Finally, after we have processed all descendents of the \textsc{Root}
node, we use the resulting term $\mathbf{z}_{\text{\textsc{Root}}}$ as
a parameter of a categorical distribution from which we draw the final
prediction label $y$:
\begin{align*}
  y \sim Cat(\mathbf{z}_{\text{\textsc{Root}}}).
\end{align*}

To represent this process graphically, we visualized probability
distributions $\mathbf{z}_{j_{k}}$ computed by the RDP for different
probability values of parent and child nodes ($\mathbf{z}_j$ and
$\mathbf{z}_k$) and present these results in
Figure~\ref{dasa:fig:rdp-alpha} (higher probabilities are highlighted
in red).

As you can see from the top-left figure, when the parent has a strong
probability of the negative class ($[1, 0, 0]$), but the probability
of the child is uniform ($[0.33, 0.33, 0.33]$), the model keeps to the
negative polarity, scooping up almost all probability mass into this
corner.  At the same, to account for the uncertainty about the child,
RDP slightly moves the summit of the probability hill (\ie{} its mean)
towards the positive class and spreads the mass more evenly along all
three axes. On the other hand, if parent and child have completely
opposite polarities (say \textsc{Negative} and \textsc{Positive}),
which the base classifier is absolutely sure of, RDP evenly
distributes the probability along the
\textsc{Positive}--\textsc{Negative} axis. (This sitation is depectied
in Subfigure~b of the picture.)  The middle row shows two symmetric
cases where parent and child again have opposite semantic
orientations, but the base predictor is less sure about its decisions
and also admits a small chance that either of these nodes is neutral.
In this case, RDP still spreads the probability along the main polar
axis, with the mean placed right in-between the two polar corners, but
additionally stretches its mass towards the center of the simplex.  As
we can see from the figure, our model does not make a distinction
between parent and child, so that the resulting probabilities in both
cases are almost identical, even though the polarities of the two
joined nodes are swapped.  Finally, in the last row, we can see our
intended discrimination of the neutral orientation: This time, the
parent node is strictly polar (negative in Subfigure~e and positive in
Subfigure~d), whereas the child is neutral.  In contrast to the
previous two examples, the mean of the resulting distribution is now
located closer to the polar corner and not in the middle of the two
juxtaposed semantic orientations.

\begin{figure}[htb]
  { \centering
    \includegraphics[width=\linewidth]{img/dirichlet-process.png} }
  \caption{Node probabilities computed by the Recursive Dirichlet
    Process}\label{dasa:fig:rdp-alpha}
\end{figure}

Our manually defined model with its fixed parameters reflects our
prior beief in the way how the joint probability of hidden and
observed states $p(y, \mathbf{z})$ might be computed.  As it turns
out, knowing this belief is sufficient to derive another probability
$q(\mathbf{z})$, which best approximates the probability of latent
variables.  In particular, we define this distribution in the same way
as above, but deprive it of the last step (drawing of the observed
label), and optimize its parameters ($\boldsymbol{\theta}$) by
maximizing the evidence lower bound, using stochastic gradient
...
values which yield the best prediction accuracy on the set-aside
development data.
...
shows very competitive numbers, yielding the best $F_1$-scores for the
...
macro-averaged result on this test set.

As it turns out, the strongest competitors of our systems are the
...
the most simplistic solutions---\textsc{Last} and \textsc{Root}.
...
decided to have a closer at their errors in order to understand their
remaining potential weaknesses.

The first such error shown in Example~\ref{snt:dasa:exmp:lcrf-error}
was made by the latent CRF system, which assigned the neutral label to
an obviously negative tweet.  As is evident from the picture of the
automatic RST tree in this example though, we can hardly expect the
right decision in this case, because neither any of the EDUs nor the
root node of this message have been considered as negative by the LBA
classifier.  But, even in this apparently hopeless situation, the
messages propagated from the leaves to the root still tell the latter
node that the predicted class better be negative. (We have inspected
the message scores passed in the forward direction and found that the
total score for the negative class amounts to 0.597, whereas the
belief in the positive class [the closest competitor] only runs up to
0.462.)  Unfortunately, these scores cannot outweigh the state
potentials of the root node, which assign the highest weight to the
neutral polarity (0.524 for the neutral class versus -0.118 for the
negative polarity).\footnote{All scores for this system are given in
...
\end{example}
As it turns out, high neutral potentials of the root node are also the
main reason for the misclassification in
Example~\ref{snt:dasa:exmp:lmcrf-error}, where the LMCRF system
...
probabilities for the two semantic orientations, so that the features
of the root node turn the scale in the final decision.
...
\end{example}
Unfortunately, the recursive Dirichlet process cannot overcome many
erroneous predictions of the base classifier either.  For instance, in
Example~\ref{snt:dasa:exmp:rdp-error}, LBA assigns the greatest scores
to the positive class in three out of four EDUs, even though each of
them by itself expresses a negative (sarcastic) attitude of the
author.  Alas, the only case where the base classifier correctly the
negative label (``Das is noch lange nicht ausdiskutiert !''
[\emph{It's no way been talked out !}]) chokes off at the very
beginning of the score propagation.  (As it turned out, the learned
$\beta$ parameter, which controls the amount of information propagated
from child to its parent, is extremely low for the negative class,
amounting to only 0.097, whereas for the positive and negative
polarities it runs up to 0.212 and 0.279 respectively.  Due to this
low value, only one tenth of the negative score from EDU 3 is taken
into account while computing the semantic orientation of the abstract
span 1, which in the end runs up to 0.265, 0.176, and 0.559 for the
negative, neutral, and positive classes respectively.)
...
Example~\ref{snt:dasa:exmp:root-error} was made by our baseline
\textsc{Root} system system, which similarly to the CRF-based
approaches confused the negative class with the neutral polarity.
This time, however, the misclassification is due to the discourse
structure itself rather than wrong predictions of the underlying
sentiment classifier.  The LBA system correctly recognizes that the
negative smiley at the end of tweet has a strictly negative semantic
orientation, but the DASA baseline can only see the first EDU at the
top of the RST tree, which merely expresses a hypothesis, free of any
polar connotation.
...
\end{example}
Although our \textsc{Last} system can correctly handle cases like the
previous example, this baseline is susceptible to other kinds of
mistakes.  For instance, in Example~\ref{snt:dasa:exmp:last-error}, it
assigns the neutral label to an obviously sad tweet based on the
polarity of the right-most discourse segment, which indeed shows some
positive attitude.  However, in contrast to the movie description from
Example~\ref{disc-snt:exmp-pang02}, where the last sentence completely
overturned the polarity of the whole preceding text, this time, the
contrasting attitude does not alter the general negative mood of the
tweet, but merely dampens its effect.
\begin{example}[Error Made by the \textsc{Last} System]\label{snt:dasa:exmp:last-error}
  \noindent\textup{\bfseries\textcolor{darkred}{Tweet:}} {\upshape
    $[$' ( :'( :'( Die letzte Aussprache war wohl das schwerste
      Telefonat meines gesamten Lebens :'( :'( :'($]_1$ $[$Aber wir gehen
        friedlich und als F $\ldots]_2$}\\
