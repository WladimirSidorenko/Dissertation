...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \State $\mathcal{S}\gets\mathcal{P}\cup\mathcal{N}$;\Comment{get set of subjective terms}
    \State $\vars{u}_{subj}, \mu_{\mathcal{S}}, \mu_{\mathcal{O}}\gets\func{FindAxis}(\vars{E}', \mathcal{S}, \mathcal{O})$;\Comment{find
      subjectivity axis}
    ...
    \EndFunction
    \Statex
    \Function{FindAxis}{$\vars{M}, \mathcal{S}_1, \mathcal{S}_2$}
    \State $\mu_1\gets 0$; $\mu_2\gets 0$; \vars{axis} $\gets 0$; \vars{max\_dist} $\gets 0$;
    \For{$\vars{i}\gets 1$; $\vars{i} <= \func{nrows}(\vars{M})$; $\vars{i}\gets\vars{i} + 1$}
    \State \vars{m} $\gets$\vars{M[i]};
    \State \vars{dist} $\gets\sum_{j_1\in\mathcal{S}_1,j_2\in\mathcal{S}_2}\left|\vars{m}[j_1] - \vars{m}[j_2]\right|$;
    \If {\vars{dist} $>$ \vars{max\_dist}}
    \State \vars{axis} $\gets$\vars{i}; \vars{max\_dist} $\gets$ \vars{dist};
    \State $\mu_1\gets\frac{\sum_{j_1\in\mathcal{S}_1}\vars{m}[j_1]}{|\mathcal{S}_1|}$; $\mu_2\gets\frac{\sum_{j_2\in\mathcal{S}_2}\vars{m}[j_2]}{|\mathcal{S}_2|}$;
    \EndIf
    \EndFor
    \State\Return\vars{axis}, $\mu_1$, $\mu_2$;
    \EndFunction
    \Statex
    \Function{ComputePolScores}{$\vars{M}, \mathcal{S}, \vars{u}_{subj}, \vars{u}_{pol},
      \mu_{\mathcal{S}}, \mu_{\mathcal{O}}, \mu_{\mathcal{P}}, \mu_{\mathcal{N}}$}
    \State\vars{scores} $\gets []$;
    \State\vars{max\_score}$_{subj}\gets\max\left(\max\left(|\vars{n}^\top - \mu_{\mathcal{S}}|,%
        |\vars{n}^\top - \mu_{\mathcal{O}}|\right)\right)\forall \vars{n}\in\func{cols}(\vars{M})$;
    \State\vars{max\_score}$_{pol}\gets\max\left(\max\left(|\vars{n}^\top - \mu_{\mathcal{P}}|,%
        |\vars{n}^\top - \mu_{\mathcal{N}}|\right)\right)\forall\vars{n}\in\func{cols}(\vars{M})$;
    ...
    \If {$|\vars{n} - \mu_{\mathcal{O}}| > |\vars{n} - \mu_{\mathcal{S}}|$}\Comment{if the $i$-th word is subjective}
    \State\vars{score}$_{subj}\gets 1 + \frac{|\vars{n} - \mu_{\mathcal{S}}|}{\vars{max\_score}_{subj}}$;
    \Else
    \State\vars{score}$_{subj}\gets 1 - \frac{|\vars{n} - \mu_{\mathcal{O}}|}{\vars{max\_score}_{subj}}$;
    \EndIf
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \If {$|\vars{n} - \mu_{\mathcal{N}}| > |\vars{n} - \mu_{\mathcal{P}}|$}\Comment{if the $i$-th word is positive}
    \State\vars{polarity}$\gets$ positive;
    \State\vars{score}$_{subj}\gets 1 \frac{|\vars{n} - \mu_{\mathcal{P}}|}{\vars{max\_score}_{subj}}$;
    \Else
    \State \vars{polarity}$\gets$ negative;
    \State\vars{score}$_{subj}\gets \frac{|\vars{n} - \mu_{\mathcal{N}}|}{\vars{max\_score}_{subj}}$;
    \EndIf
    \State\func{append}(\vars{scores},
    (\vars{i}, \vars{polarity}, $\vars{score}_{\textrm{subj}} + \vars{score}_{pol}$));
    \EndFor
...
projected words, we considered the sum of these values as the final
lexicon score when including a word into the resulting polarity list.
The pseudo-code of this approach is shown in
...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{p{0.11\columnwidth} % first columm
      ...
      % Class                     Precision              Recall                 F-score
      % 80
      % positive                   0.588926             0.103448                 0.175984
      % negative                   0.457143             0.017884                 0.034421
      % neutral                    0.963216             0.998216                 0.980404
      % Macro-average              0.669762             0.373183                 0.396936
      % Micro-average              0.961416             0.961291                 0.961353
      ...
      %% file: data/results/knn/knn.vectors.word2vec.turney_littman_2003.txt
      % Computed using:
      % ./scripts/eval_cardinality data/results/knn/knn.word2vec.turney_littman_2003.txt
      % Class                     Precision              Recall                 F-score
      % 5120
      % positive                   0.028954             0.101975                 0.045102
      % negative                   0.005488             0.017139                 0.008314
      % neutral                    0.957841             0.865014                 0.909064
      % Macro-average              0.330761             0.328042                 0.320826
      % Micro-average              0.833480             0.833317                 0.833398
      ...
      % Class                     Precision              Recall                 F-score
      % 40
      % positive                   0.672381             0.104038                 0.180194
      % negative                   0.567901             0.017139                 0.033273
      % neutral                    0.963204             0.998758                 0.980659
      % Macro-average              0.734495             0.373312                 0.398042
