...
After obtaining the results for the most popular sentiment analysis
approaches with the (mostly) standard settings, we decided to
investigate the impact of different external factors on the net scores
of these methods.  For this purpose, we reran the evaluation, changing
one aspect of the training data or procedure at a time, and
reestimated the scores on the development set (in order to keep the
test corpus undisclosed).  The results of these experiments are
presented below.
...
assign the \texttt{sentiment} tag to complete syntactic or
discourse-level units which encompassed both the target of an opinion
and its immediate evaluation.  Even though this decision is
linguistically plausible and extremely helpful for determining the
boundaries of opinions, it also poses considerable difficulties for
sequence labeling approaches, since the same tag gets assigned not
only to immediately subjective words, but also to objective terms
which reside within the same syntactic constituent as the polar term
and its respective target.  Since none of the tested methods could
explicitly incorporate this logic, we decided to check whether an
...
of the annotation scheme), we only assigned this label to emotional
expressions found in the corpus (which we dub a \emph{narrow}
interpretation of the scheme).  The difference between these two ways
of interpreting the annotation scheme is shown in
...
sentence with the \texttt{SNT} tag except for the words which denoted
the target and source of the opinion.  In the latter (narrow) case, we
only assigned the \texttt{SNT} tag to the emotional expression
``good'' and the emoticon ``:)'', which, however, were expressive
enough to convey the main evaluative sense of the whole subjective
statement.

The results of the automatic systems with these two iterpretations are
...
lower~(cf., for instance, 0.31~\F{} attained by the linear-chain CRFs
with the broad interpretation versus 0.24~\F{} achieved by this model
with the narrow mapping).

An obvious explanation for these results is the expectedly better
amenability of the narrow scheme to the prediction of sentiment labels
(since \texttt{sentiment} tags only get assigned to unequivocally
polar terms, it becomes easier for the models to infer this class
using state features---especially morphological or lexical ones---or
the learned embeddings).  However, on the other hand, such short spans
lead to disrupted label chains for opinion-related elements, making
sentiment tags be far apart from the spans of the respective sources
and targets.  Consequently, these latter classes suffer from the lack
of context and become heavily dependent on the state attributes too.
Although, this time, the effect of these features is rather negative,
since, in contrast to polar terms, being a holder or target of an
opinion is not an inherent property of a lexical expression, but
arises from the context which this expression appears in.  Consider,
for instance, the name ``Silvio Berlusconi'' in
...
objective statement in the second case.  As we can see from this
example, the decision about the role of this name depends primarily on
the sense of the whole sentence rather than only the name.  Therefore,
state attributes might only increase our prior belief that certain
words appear in subjective rather than objective contexts (e.g., one
of the most frequent mistakes made by the CRF was always to assign the
target tag to the word ``Nordkorea'' (\emph{North Korea}) regardless
of the sentence content), but cannot tell us for sure whether they
actually do or not.  This is especially so, when the actual polar term
appears at the other end of the sentence.
...
Since the lack of contextual links appeared to have a strongly
negative effect on the prediction of surces and targets of opinions,
we decided to investigate whether redefining the way these links were
established in the model could help improve the results.  For this
purpose, we implemented three possible extensions to the traditional
first-order linear-chain CRF model:
\begin{itemize}
  \item higher-order linear-chain CRFs,
  \item first- and higher-order semi-Markov models, and
  \item tree-structured CRFs.
\end{itemize}
In the first of these variants, the standard binary transition
functions $f(\mathbf{x}_m, y'', y')$:
\begin{equation*}
  f(\mathbf{x}_m, y'', y') = \begin{cases} \phi(\mathbf{x}_m), &
    \mbox{if } \mathbf{y}_{m-1} = y''\mbox{ and }\mathbf{y}_{m} =
    y'\\ 0, & \mbox{otherwise;}
  \end{cases}
\end{equation*}
was replaced with a sum over multiple predicates up to an arity of
$n$, where $n- 1$ is the order of the model.  For instance, a typical
transition feature for the third order linear-chain CRF would look as
follows:
\begin{equation*}
  f(\mathbf{x}_m, y'''', y''', y'', y') = \begin{cases} \phi(\mathbf{x}_m) + f(\mathbf{x}_m, y''', y'', y'), &\mbox{if } \mathbf{y}_{m-3} = y''''\mbox{ and }\mathbf{y}_{m-2} = y'''\\
    &\mbox{ and }\mathbf{y}_{m-1} = y''\mbox{ and }\mathbf{y}_{m} =
    y'\\ 0, & \mbox{otherwise;}
  \end{cases}
\end{equation*}
Since transition features had to be consistently multiplied with the
scores of the previous states while doing forward propagation, the
number of states (each of which kept the score of a particular
preceding label sequence) increased exponentially with the order of
the model.  To mitigate this effect and keep the inference still
tractable, we applied the algorithm of~\citet{Nguyen:14}, reducing the
number of states only to the sequence which were actually observed in
the training set.

The same heuristic approach was also used for higher-order semi-Markov
CRFs, which, in contrast to linear-chain models, simultaneously try to
find both the most likely segmentation and the label assignment to the
segments.  In particular, instead of computing the conditional
probability of a label sequence given the input
\begin{equation*}
  P(\mathbf{y}|\mathbf{x}) = \frac{\exp\left(\sum_{m=1}^{M}\sum_jw_{j}
      \times f_j(x_{m}, y_{m-1},
      y_{m})\right)}{Z\left(\mathbf{x}\right)},
\end{equation*}
semi-Markov models estimate the probability of a segmentation, in
which each segment is associated with a particular label:
\begin{equation*}
  P(\mathbf{s}|\mathbf{x}) = \frac{\exp\left(\sum_{n=1}^{N}\sum_jw_{j}
      \cdot f_j(s_{n}, y_{s_{n-1}},
      y_{s_n})\right)}{Z\left(\mathbf{x}\right)},
\end{equation*}
where $\mathbf{s}$ stands for the whole segmentation of the input
instance, $s_{n}$ denotes the $n$-the segment, and $y_{s_{n-1}}$ and
$y_{s_n}$ represent the labels of the previous and current segments
respectively.  The normalization factor $Z\left(\mathbf{x}\right)$ is
computed over all possible label and segment assignments, with
segments' length ranging from 1 to $K$, where $K$ is the maximum
length of a contiguous tag span observed in the training data.

The tree-structured CRFs represent a generalization of the
linear-chain case, in which transition links are established between
syntactic dependants instead of adjacent tokens.  We derived the
netwotk graphs from the automatically constructed Mate dependency
trees.  Since these tree structures are acyclic, one can still apply
the belief propagation method (where the incident scores coming from
the child nodes are summed up), getting both a convergence guarantee
during the training and an exact inference during the testing time.

The results of these systems on the training and development sets are
shown in Table~\ref{fgsa:tbl:crf-topologies}.  As we can see from the
scores, semi-Markov CRFs are generally better at predicting sentiments
and targets.  Moreover, increasing the order of the models up to the
second degree also improves the results for sources and targets of
opinions.  However, higher-order approaches again start suffering from
overfitting the training data.  Somewhat counterintuively,
tree-structured CRFs get worse results than the linear chain model,
which can partially be explained by errors in the syntactic trees
which are produced on this domain.
\begin{table*}
  \begin{center}
    \bgroup \setlength\tabcolsep{0.1\tabcolsep}\scriptsize
...
In a similar way as we did for CRFs, we also implemented higher-order
and tree-structured models for the common RNN methods---LSTM and GRU.
In particular, we passed a concatenation of $n$ preceding $\vec{h}$
vectors as input to the reccurrence loop in the higher-order case
(where $n$ stands for the order of the model), and constructed the
recurrence schedule from the automatic dependency graphs for the
tree-structured models.

The results of these systems are shown in
Table~\ref{fgsa:tbl:nn-topologies}.
...
% Tag        Precision    Recall F-Measure
% O             86.21%    87.44%    86.82%
% SENTIMENT     57.93%    72.23%    64.29%
% SOURCE        46.62%    69.97%    55.95%
% TARGET        43.22%    76.01%    55.11%
...
% Tag             Precision    Recall        F1
% SENTIMENT           49.86     74.54     58.44
% SOURCE              44.81     63.66     52.54
% TARGET              41.09     72.59     52.12
% Macro-F1  54.3678
...
% Tag             Precision    Recall        F1
% SENTIMENT           51.00     65.95     56.99
% SOURCE              42.32     62.30     50.34
% TARGET              46.65     62.75     51.92
% Macro-F1  53.0844
...
% Tag        Precision    Recall F-Measure
% O             82.75%    88.99%    85.75%
% SENTIMENT     61.09%    64.14%    62.57%
% SOURCE        43.26%    72.45%    54.17%
% TARGET        43.88%    72.01%    54.53%

% Tag        Precision    Recall F-Measure
% O             84.05%    86.07%    85.05%
% SENTIMENT     54.56%    73.06%    62.47%
% SOURCE        45.20%    68.61%    54.50%
% TARGET        43.81%    73.83%    54.99%

% Tag        Precision    Recall F-Measure
% O             87.02%    78.11%    82.32%
% SENTIMENT     37.04%    81.82%    50.99%
% SOURCE        47.12%    66.16%    55.04%
% TARGET        42.89%    71.68%    53.66%

% Tag             Precision    Recall        F1
% SENTIMENT           50.90     73.01     58.68
% SOURCE              45.19     69.07     54.57
% TARGET              43.53     72.51     54.39
% Macro-F1  55.8800

% Dev Set
%Tag        Precision    Recall F-Measure
% O             77.32%    85.60%    81.25%
% SENTIMENT     29.84%    19.67%    23.71%
% SOURCE        23.05%    40.50%    29.38%
% TARGET        22.66%    22.46%    22.56%

% Tag        Precision    Recall F-Measure
% O             76.63%    82.50%    79.46%
% SENTIMENT     25.98%    23.57%    24.72%
% SOURCE        24.13%    37.83%    29.47%
% TARGET        20.55%    21.98%    21.24%

% Tag        Precision    Recall F-Measure
% O             76.84%    75.21%    76.01%
% SENTIMENT     22.91%    33.65%    27.26%
% SOURCE        26.93%    35.63%    30.68%
% TARGET        21.46%    22.19%    21.82%

% Tag             Precision    Recall        F1
% SENTIMENT           26.24     25.63     25.23
% SOURCE              24.70     37.99     29.84
% TARGET              21.56     22.21     21.87

%% Order 3
% Train Set
% Tag        Precision    Recall F-Measure
% O             86.39%    83.76%    85.05%
% SENTIMENT     43.74%    80.02%    56.56%
% SOURCE        46.65%    68.83%    55.61%
% TARGET        51.08%    74.50%    60.60%

% Tag        Precision    Recall F-Measure
% O             81.66%    90.63%    85.91%
% SENTIMENT     60.13%    67.51%    63.61%
% SOURCE        42.86%    72.56%    53.89%
% TARGET        55.02%    66.83%    60.35%

% Dev Set
% Tag        Precision    Recall F-Measure
% O             76.09%    78.42%    77.24%
% SENTIMENT     24.53%    28.49%    26.36%
% SOURCE        27.11%    37.00%    31.30%
% TARGET        21.22%    19.30%    20.22%

% Tag        Precision    Recall F-Measure
% O             75.96%    87.42%    81.29%
% SENTIMENT     29.55%    21.46%    24.86%
% SOURCE        23.39%    41.33%    29.87%
% TARGET        23.64%    14.40%    17.90%

\begin{table*}
  \begin{center}
    \bgroup \setlength\tabcolsep{0.1\tabcolsep}\scriptsize
    \begin{tabular}{p{0.12\columnwidth} % first columm
        *{8}{>{\centering\arraybackslash}p{0.1\columnwidth}}} % next nine columns
      \toprule
      \multirow{2}*{\bfseries Element} & %
      ...
      Sentiment & 0.584 &  &  & 0.57 & 0.587 &  &  &  \\
      Source & 0.525 &  &   & 0.503 & 0.546 &  &  &  \\
      Target & 0.521 &  &  & 0.519 & 0.544 &  &  &  \\

      \multicolumn{9}{c}{\cellcolor{cellcolor}Development Set}\\

      Sentiment & 0.278 &  &  & 0.335 & 0.252 &  &  &  \\
      Source & 0.328 &  &   & 0.263 & 0.298 &  &  &  \\
      Target & 0.259 &  &  & 0.216 & 0.219 &  &  &  \\\bottomrule
    \end{tabular}
    \egroup
    \caption{Results of fine-grained sentiment analysis with different
      neural network topologies.\\ {\small lcLSTM---linear-chain LSTM,
        lcGRU---linear-chain GRU,\\trLSTM---tree-structured LSTM,
...
% Tag        Precision    Recall F-Measure
% O             76.42%    79.33%    77.84%
% SENTIMENT     30.65%    41.05%    35.09%
% SOURCE        31.31%    28.12%    29.63%
% TARGET        22.92%    29.15%    25.66%
...
% Tag        Precision    Recall F-Measure
% O             78.82%    68.16%    73.11%
% SENTIMENT     24.28%    35.71%    28.90%
% SOURCE        21.66%    31.72%    25.74%
% TARGET        18.92%    32.88%    24.02%

% Tag        Precision    Recall F-Measure
% O             77.90%    73.98%    75.89%
% SENTIMENT     28.01%    34.43%    30.89%
% SOURCE        17.89%    37.72%    24.27%
% TARGET        20.90%    29.70%    24.54%

% Tag        Precision    Recall F-Measure
% O             76.30%    81.36%    78.75%
% SENTIMENT     31.94%    28.91%    30.35%
% SOURCE        22.15%    31.90%    26.14%
% TARGET        21.02%    22.44%    21.71%
