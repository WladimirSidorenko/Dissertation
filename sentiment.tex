...
destinations into \emph{recommended} (thumbs up) and \emph{not
  recommended} (thumbs down) ones.  These decisions were made by
taking the sum of the pointwise mutual information (PMI) scores
between the adjectival and adverbial phrases occurring in the review
text and the word ``excellent'' and then subtracting from it the PMIs
of these phrases in conjunction with the word ``poor''.

As noted by \citet{Turney:02}, movie critiques were by far the most
