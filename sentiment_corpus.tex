...
\cite{Cohen:60}---binary and proportional kappa---that have been
...
one also needs to consider various in-domain factors, which might
...
of opinionful topics to us were \emph{the papal conclave}, which took
place in March of that year, and \emph{the German federal elections},
which were held in autumn.  Since both of these events implied some
form of voting, we decided to counterbalance the election specifics by
including \emph{general political discussions} as the third subject in
our dataset.  Finally, to obey the second principle, \ie{} to keep the
corpus bias low, we sampled the rest of the data from \emph{casual
  everyday conversations} without any prefiltering.
...
extensive list of elements that had to be annotated by the experts.
This list included:

\begin{itemize}
\item
  \textbf{\markable{sentiment}}s, which were defined as \emph{polar
    subjective evaluative opinions about people, entities, or
    events}. In these labels, our annotators had to include both the
  entities that were evaluated and their respective evaluative
  expressions. Apart from tagging the text spans of these elements,
  they also had to specify the following tag attributes:
  \begin{itemize}
  \item\attribute{polarity}, which reflected the attitude of the
    opinion's holder to the evaluated entity.  Following
    \citet{Jindal:06a,Jindal:06b}, we distinguished between
    \emph{positive}, \emph{negative}, and \emph{comparative} semantic
    orientations;
...
\item
  another element in our scheme was \textbf{\markable{target}}s, which
  denoted \emph{entities or events evaluated by opinions}.  For this
  item, we introduced the following three attributes:
  \begin{itemize}
    \item
    ...
    \item
      ...
      antecedent of a pronomial target;
    \item and, finally, another edge feature,
      \attribute{sentiment-ref}, which linked \markable{target}s to
      their respective \markable{sentiment}s in the cases when the
      \markable{target} span was located at the intersection of two
      opinions;
  \end{itemize}

\item
  another important component of \markable{sentiment}s,
  \textbf{\markable{source}}s, signified \emph{the immediate author(s)
    or holder(s) of opinions}.  The only property associated with this
  element was \attribute{sentiment-ref}, which was defined the same
  way as for \markable{target}s.
\end{itemize}
...
A sample tweet annotated according to this rule is shown in
...
pronoun syntactically dependends on the target word
``Milliardeneinnahmen'' (\emph{billions of revenue}).

Apart from \markable{sentiment}s, \markable{target}s,
\markable{source}s, we additionally asked the annotators to label
elements that could significantly affect the intensity and polarity of
an opinion.  These elements were:

\begin{itemize}
\item
  \textbf{\markable{polar term}}s, which were defined as \emph{words
    or idioms that had a distinguishable evaluative lexical meaning}.
  ...
  intensity), whereas \markable{sentiment}s' attributes had to show
  the compositional meaning of the whole opinion (\ie{} its
  \emph{contextual} polarity and intensity).

  Apart from these common properties, \markable{polar term}s also had
...
\item
  ...
  \textbf{\markable{intensifiers}}.  Typical examples of such
  expressions were adverbial modifiers such as ``sehr'' (\emph{very}),
  ``super'' (\emph{super}), ``stark'' (\emph{strongly});

\item
  \textbf{\markable{diminishers}}, on the contrary, were \emph{words
    or phrases that decreased the polar lexical sense of a polar
    term}.  Like \markable{intensifier}s, these elements were usually
  expressed by adverbs, \eg{} ``weniger'' (\emph{less}), ``kaum''
  (\emph{hardly}), ``fast'' (\emph{almost}).

  Both of these elements (\markable{intensifier}s and
  ...
  which connected the modifier to its respective
  \markable{polar-term};

\item the final element, \textbf{\markable{negation}}s, was defined as
  \emph{grammatical or lexical means that reversed the polarity of a
    polar term}.  These were typically represented by the negative
  particle ``nicht'' (\emph{not}) or indefinite pronoun ``keine''
  (\emph{no}).  The only attribute associated with this tag was a
  mandatory link \attribute{polar-term-ref}.
\end{itemize}
...
only single words without their grammatical dependents.
...
main verb phrase simultaneously covers both the target of the
evaluation (``Die Nazi-Vergangenheit'' [\emph{The Nazi history}]) and
its respective polar expression (``nicht sehr r\"uhmlich'' [\emph{not
    very laudable}]).  The boundaries of \markable{sentiment} and
...
other hand, only mark single words.  The polarity of the term (its
primary semantic orientation without the context) is \emph{positive}.
...
second annotators respectively, and the $M_1$ and $M_2$ terms mean the
numbers of tokens with matching annotations.  As usual, we computed
the chance agreement $p_c$ as:
...
metrics: \emph{binary} and \emph{proportional} kappa.  With the former
variant, we counted tokens belonging to overlapping annotation spans
of the same class multiple times (\ie{} $A_1$ and $A_2$ would amount
to $10$ and $9$ respectively in the above tweet) and considered all
tokens belonging to the given annotated element as matching if this
span agreed on at least one token with the annotation from the other
expert (\ie{} $M_1$ and $M_2$ would have the same values as $A_1$ and
$A_2$ in this case).  With the latter metric, every labeled token was
counted only once (\ie{} the numbers of labeled words in the first and
second annotations would be $7$ and $6$ respectively), and we only
...
Example~\ref{example:snt:iaa} would consequently be 1.0, because this
metric would consider both annotations as perfectly matching, since
every labeled \markable{sentiment} agreed on at least one token with
the other annotation.  The proportional kappa of these labels,
...
\begin{itemize}
  ...
  \item In the second step, in order to improve the inter-rater
    ...
    30.73\%, reaching 61.94\%.  Similar effects were also observed for
  ...
  \item After resolving all differences, our assistants proceeded with
    ...
    independently, one of the experts annotated 78.8\% of the full
    corpus, whereas the second annotator labeled the complete dataset.
\end{itemize}
...
\markable{target}s show worse results than the scores of
\markable{source}s: $31.21\%$ and $26.85\%$ versus $33.75\%$ for
\markable{source}s.  We explain this difference by the fact, that
\markable{sentiment}s and \markable{target}s are typically represented
by syntactic or discourse-level constituents (noun phrases or clauses)
and, even though experts might agree on the presence of these elements
more often (as suggested by the binary-$\kappa$ metric), reaching a
consensus about the exact boundaries of these elements turns out to be
a challenging task despite an explicit clarification of this problem
in the annotation guidelines; \markable{source}s, on the other hand,
are usually expressed by pronouns, which rarely accept syntactic
attributes and whose boundaries are therefore easier to determine.
Nevertheless, even with the binary metric, the agreement of all
sentiment-level elements is significantly below the $40\%$ threshold,
which means only a slight reliability according to the
\citeauthor{Landis:77} scale \cite{Landis:77}.

A different situation is observed for \markable{polar terms} and
\markable{intensifiers}.  The inter-annotator agreement on these
elements is above 60 and 50\% respectively, for both variants of the
proposed $\kappa$-measure.  Obviously, defining these entities as
lexical units has significantly eased the detection of their
boundaries.  This effect becomes even more evident when we look at
\markable{diminisher}s and \markable{negation}s, where the $A$ and $M$
scores are absolutely identical for both metrics.  It means that both
annotators always agreed on the boundaries of these elements whenever
they agreed on their presence.  Unfortunately, due to a rather small
number of these tags in the corpus (with only 3 cases of
\markable{diminisher}s and 21 cases of \markable{negation}s), the
overall agreement on these elements is relatively small too, amounting
...
all annotated elements: the binary scores of sentiments and targets
increased by $29.87\%$ and $30.18\%$ respectively.  An even greater
improvement is observed for sources, whose binary kappa rose by
remarkable $38.38\%$.  A similar tendency applies to the proportional
metric: here, the agreement on \markable{sentiment}s gained $30.73\%$,
reaching $61.94\%$.  Likewise, the reliability of opinion targets and
holders improved by $30.42\%$ and $37.37\%$, running up to $57.27\%$
and $71.12\%$ respectively.

In general, however, we can see that the second annotator tended to
label almost twice as many \markable{sentiment}s and
\markable{target}s as the first expert: the proportional $A_1$ scores
of these two entity types amount to 8,243 and 3,326 tokens, whereas
the corresponding $A_2$ counts run up to 13,714 and 5,212 words.
...
benchmark, which means an almost perfect level of agreement.
...
and proportional scores of \markable{polar terms} only amounts to
...
that the majority of these disagreements stemmed from a different
treatment of the exclamation mark: the first expert skipped these
punctuation marks during the annotation, whereas the second coder
considered them as valid intensifying elements.  Nevertheless, even
despite these diverging interpretations, the reliability on
\markable{intensifier}s is above $55\%$, which is considered as a
moderate agreement.
...
of coding, we eventually let them label the remaining part of the
...
whereas the agreement figures for \markable{target}s were more
persistent and lost only $0.66\%$, going down to $56.61\%$.  The most
dramatic changes occurred for \markable{source}s, whose proportional
value deteriorated by notable $7.02\%$, sinking to $64.1\%$.
Nonetheless, the average proportional agreement of all these elements
is around $60.5\%$, which is almost twice as high as the mean
...
decrease: the agreement on \markable{intensifier}s deteriorated by
5.8\%, sinking to 49.71\% proportional $\kappa$.  A similar situation
is observed for \markable{diminisher}s, whose $\kappa$-rank worsened
from $58.05\%$ to $50.78\%$.  The best persistence in this regard is
shown by \markable{negation}s, whose quality dropped by only $1.11\%$,
...
decided to have a closer look at diverging annotations.  A sample
sentence with different analyses of \markable{sentiment}s is shown in
...
\end{example}
In this tweet, the first coder erroneously overlooked the emoticon
\smiley{} at the end of the message, whereas the second expert
correctly recognized that this smiley evaluates the content described
in the sentence.  Because of these different decisions about the
presence of a \markable{sentiment}, the annotators trivially disagreed
on the annotation of the \markable{target} element as well.
...
the experts actually agreed on the \markable{sentiment} spans.  An
example of such situation is shown in the following message:
...
We should, however, note that these different takes of subjective
facts were to some extent due to the adjudication procedure that we
applied in step two, because a closer analysis of these cases revealed
that, at the initial stage, our experts had opposite preferences
regarding these entities (viz., the first annotator typically
annotated them, whereas the second coder usually skipped these words).
During the revision, however, both assistants interpreted their
decisions as false, and both changed their minds.  Even though the
rest of their changes made in that step has still significantly
improved the results, the possibility of mutual concessions needs to
be kept in mind when applying the adjudication method in future.
...
%% terms as their contextual valence instead, \ie{} the polarity of the
...
these elements in our data (see
...
formal traits has notably influenced both the number of the opinion
elements and the difficulty of their interpretation.  According to
Figure~\ref{snt:fig:crp-sent-emo-distr}, tweets pertaining to the
federal elections and topically unfiltered messages were the ones with
the highest number of the annotated sentiments.  A similar tendency is
also observed for the emotional expressions, although, in this case,
the topical grouping appeared to play a less important role than the
formal criteria.  Moreover, a higher number of opinionated terms might
not necessarily lead to a higher number of targeted sentiments.  We
can recognize that from the fact that, even though the biggest number
of subjective opinions show up in the first row of
...
Summarizing all of the above, we would like to remind the reader that
in this chapter we presented a corpus of 7,992 German tweets, which
had been manually annotated by two human experts with sentiments.

To collect the initial data for this dataset, we were tracking
...
conversations between spring and autumn 2013.  From these messages, we
generated groups of microblogs containing polar terms, emoticons, and
none of these entities, sampling in the end 666 posts from each of
these categories for each of the four topics.
