...
Twitter messages.  In order to estimate how aggravated this problem really is,
we collect statistics on the most frequent out-of-vocabulary words found in
sample users' messages, classify these words into three major classes (regular
...
spelling mistakes).  %% In the description of the related work, we also juxtapose
...
sentiment analysis of sentences.  Here, we first present a comprehensive corpus
of German microblogs which were manually annotated with fine-grained sentiment
relations.  We then conduct an agreement study of this corpus in order to
evaluate how difficult the sentiment analysis task is for humans and which
sentiment elements, text topics, and tweet types are most difficult to
annotate.  After that, we demonstrate how well the sentiment classification
task can be done automatically using wide-spread machine learning techniques.
In our experiments, we consider three related subtasks:
\begin{inparaenum}[\itshape a\upshape)]
...
and discourse structures in users' messages.
