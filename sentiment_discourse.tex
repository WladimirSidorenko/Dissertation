...
\begin{figure}[htbp]
  \begin{center}
    \begin{tikzpicture}[>=triangle 45,semithick]
      ...
      \draw[->] (p'') to node [xshift=-8mm,yshift=0mm] {Commentary} (1c);
...
we should use for our further sentiment experiments, we motivated our
choice by theoretical and practical considerations: From the
theoretical perspective, we wanted to have a strictly hierarchical
discourse structure for the analyzed tweets.  From the practical
aspect, since there was no disourse parser readily available for
...
native data.  Fortunately, both of these concerns lead us to the same
solution---rhetorical structure theory, which, on the one hand, was
the only formalism which explicitly imposed the tree constraint on the
resulting discourse graph, and, on the other hand, offered a wide
variety of open-source parsing systems \cite[i.a.,
][]{Hernault:10,Feng:14,Ji:14,Yoshida:14,Joty:15}.
