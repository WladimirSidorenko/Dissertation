...
\subsection{Fine-Grained Sentiment Analysis with Conditional Random
  Fields} The first automatic method that we are going to evaluate in
this section is that of the conditional random fields (CRFs).  First
...
resons for such huge success are:
\begin{enumerate}[1)]
...
\item and, finally, the \emph{avoidance of the label bias problem}, to
  ...
  \end{example}
\end{enumerate}
\textbf{Training} CRFs possess these useful properties thanks to a
...
denotes its gold labels, CRF's training adds up to finding feature
