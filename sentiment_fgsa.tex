...
included both the object of an evaluation---``der neue Papst''
(\textit{the new pope})---and the evaluation itself---``bescheidener,
zur\"uckgenommener Typ'' (\textit{a sober, modest man}).

We used the same principles of minimality and completeness for the
...
its dependants without including any of its parental elements.

\subsection{Evaluation Metrics}
The next natural question which arises after defining the span
...
the quality of an automatic assignment is to compute the precision,
...
both of these approaches are relatively problematic: While the binary
...
In their work, the authors devised another measure for estimating the
quality of an automatic assignment, in which they penalized the
predicted spans proportional to the number of tokens whose labels were
different from the gold annotation.  More precisely, given a set of
manually annotated gold spans $\mathcal{S}$ and automatically tagged
entities $\widehat{\mathcal{S}}$, they estimated the precision of the
...
$s_j$; and $|\widehat{\mathcal{S}}|$ meant the total number of tokens
automatically labeled with the given tag.  Similarly, the recall of
such assignment was estimated as:
\begin{equation*}
...
\end{equation*}
Since this metric adequately accommodates both extrema of an automatic
assignment---too long and too short spans---and also penalizes for
erroneous and spurious labels, we will rely on it throughout our
...
\item the \emph{structural nature} of CRFs, which, in contrast to
  ...
  most likely sequence of dependent valiables corresponding to these
  observations;
\item the \emph{discriminative power} of this framework, which, in
  contrast to generative probabilistic graphical models such as HMM
...
\begin{example}[Overlapping and Correlated Features]
  ...
  ``von der Leyen'' in the test example and training data and rerun
  ...
  logistic regression: In contrast to generative frameworks, which
  optimize the joint distribution $P(\mathbf{x}, y)$, discriminative
  classifiers seek to optimize the conditional likelihood
  $P(y|\mathbf{x})$ by maximizing the probability of the training set
  $\sum_{i=1}^N\log P(y_i|\mathbf{x}_i, \mathbf{\lambda})$, where $N$
  is the number of training instances, $y_i$ is the gold label for the
  $i$-th instance (e.g., 1 for \texttt{CDU} and 0 for \texttt{SPD}),
  $\mathbf{x}_i$ stands for the features of this instance,
  $\mathbf{\lambda}$ are the learned feature weights, and the
  probability $P$ being normally computed as the sigmoid function
  $\frac{1}{1 + e^{-(\mathbf{x}_i, \mathbf{\lambda})}}$.  Working
  through the derivatives of these features, we will arrive at the
  solution $\lambda_1 \approx 0.5052$ for the feature ``Merkel'' and
  $\lambda_2 \approx -0.5052$ for the feature ``Steinmeier'' for the
  first example, which would again result in equal 50\% probabilities
  for both classes.  In the second example, however, all three
  features ``von'', ``der'', and ``Leyen'' will get an equal weight of
  $\approx 0.2936$, and the ``Steinmeier'' feature will receive a
  coefficient of $\approx -0.4364$, which would result in 60\% chances
  for the message being about the CDU and 40\% probability that the
  tweet is about the SPD.  Even though this still means a slight
  skewness towards \texttt{CDU}; this time, the effect of correlated
  features is much less dramatic than in the generative case.
\end{example}
