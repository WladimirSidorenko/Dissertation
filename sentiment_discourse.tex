...
\begin{align*}
  scale&= \frac{\varsigma \times \left(0.1 + \cos\left(\mathbf{z}^*_j,
    \mathbf{z}_{i_{j-1}}\right)\right)}{H\left(\boldsymbol{\alpha}_{i_j}\right)};
\end{align*}
where $\varsigma$ is a random model parameter drawn from a
$\chi^2$-distribution: $\varsigma\sim\chi^2(34)$; 0.1 is a constant
used to prevent zero scales in the cases when
$\cos\left(\mathbf{z}^*_j, \mathbf{z}_{i_{j-1}}\right)$ is equal to
zero; and $H\left(\boldsymbol{\alpha}_{i_j}\right)$ stands for the
