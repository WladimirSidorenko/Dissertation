...
usually dominates its grammatical dependents; and, in discourse, one of
...
segmented discourse representation explicitly allows the text structure
...
three discourse units are extremely rare, which is also not surprising
...
the most important context factors which could significantly affect the
...
these nodes to the overarching vertex representing the polarity of the
...
the help of Markov logic constraints; and, finally, \citet{Wang:13},
...
ascribe one of four opinion categories (reporting, judgment, advice,
...
solutions, which will be briefly described below.
...
irrespective of the input).  Since the \textsc{Root} vertex,
...
introduced by discourse relations; for example, a contrastive link
...
opinion does not alter the general negative mood of the message, but
merely dampens its strength.
...
key factors:
\begin{inparaenum}[(i)]
\item the scores predicted by the base sentiment system for messages
  and EDUs and
...
decided to rerun our experiments from the previous section, trying
...
0.2\% worse result than RDP (0.716 versus 0.718 macro-\F{}).  Despite
its significant improvements, LMCRF cannot beat the micro-\F{}--score
of the discourse-unaware baseline either, even though it achieves the
same \F{}-results for the positive and neutral classes as the
discourse-unaware method.  The most surprising finding for us though is
that in gold discourse annotation, EDUs which determine the actual
polarity of the tweet are unlikely to appear either at the end of a
message or at the top of its RST tree, which explains the degradation
of the scores for the \textsc{Last} and \textsc{Root} approaches.
...
\citet{Heerschop:11}, and \citet{Zhou:11}, as well as the original set
of RST links from the Potsdam corpus.  A detailed overview of these
schemes is given in Table~\ref{dasa:tbl:rst-rel-sets}.
\begin{table}[h]
  \begin{center}
    \bgroup
...
in Table~\ref{dasa:tbl:dplp-results}.
\begin{table}[h]
  \begin{center}
    \bgroup \setlength\tabcolsep{0.1\tabcolsep}\scriptsize
...
As is evident from the scores, coarser relation schemes indeed improve
parsing quality, especially in terms of relation \F{}.  In the most
extreme case (\eg{} \citeauthor{Bhatia:15} [which has only two links]
versus PCC [which comprises 34 relations]), these gains can reach up
to seven percent. However, with respect to other metrics (span and
nuclearity \F{}), the gaps are notably smaller and might even be in
favor of the richer set (cf. nuclearity \F{} for PCC).
...
reweighting strongly benefits from the full unconstrained PCC set,
which is apparently due to the better nuclearity prediction achieved
with this scheme.  Finally, \textsc{WNG} and \textsc{Root} reach their
best results with the relation subsets proposed
by~\citeauthor{Chenlo:14} and \citeauthor{Heerschop:11} respectively.
...
\begin{itemize}
  ...
  \item to substantiate our claims and to see whether the
    ...
    would indeed benefit from awareness on discourse structure, we
    ...
    parser of~\citet{Ji:14}, which we had previously retrained on the
    Potsdam Commentary Corpus~\cite{Stede:14};
  \item afterwards, we estimated the results of existing
    ...
    of a tweet by taking the polarity of its last and root EDUs), on
    these automatically parsed data, getting the best results with the
    R2N2 solution of~\citet{Bhatia:15} (0.657 and 0.559 macro-\F{} on
    PotTS and SB10k respectively);
  \item we could, however, improve on these results and also
    outperform the plain LBA system with our three proposed
    discourse-aware solutions (latent and latent-marginalized
    conditional random fields and recursive Dirichlet process),
    boosting the macro-averaged \F{}-score on PotTS to 0.678 and
    increasing the result on SB10k to 0.56 macro-\F{};
  \item a subsequent evaluation of these approaches with different
    ...
    largely correlated with the scores of base sentiment classifiers
    (the systems we used to predict the polarities of single EDUs) and
    also revealed an important drawback of the latent-marginalized
    CRFs, which failed to predict any negative instance on the test
    set of the SB10k corpus when used in combination with the
    lexicon-based approach of~\citet{Hu:04};
