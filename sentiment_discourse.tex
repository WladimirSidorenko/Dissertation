...
sentences might eventually constitue a discourse.  Furthermore,
besides this inherent structural heterogeneity, even units of the same
linguistic level might play a different role and be of unequal
importance to the meaning of the higher-level whole: for example, in
words, the root morpheme typically conveys more lexical information
than affixes; in sentences, the syntactic head usually dominates all
its grammatic dependents; and the same kind of imbalance can also be
observed in discourse, where one of the sentences frequently expresses
the gist of the whole text.
...
sentiment analysis (DASA), also presenting several own DASA approaches
and eventually evaluating their results on the aforementioned corpora.
After analyzing the effects of different common factors (such as
various subsets of discourse relations that might be used in a DASA
system, and the quality of the downstream sentiment classifier), we
will recap our results and draw conclusions in the last section of
this final chapter.
...
its segmentation into \emph{elementary discourse units} (EDUs) and the
induction of (hierarchical) \emph{coherence relations} between these
EDUs.

Admittedly, the idea of splitting the discourse into smaller
meaningful pieces and inferring semantic links between those parts is
anything but new and dates back to the very origins of general
linguistics \cite{Aristotle:10} and in particular its structuralism
branch \cite{Saussure:90}.  However, an especially big surge of
interest in this field happened in the 1970-s with the groundbreaking
works of \citet{vanDijk:72} and \citet{vanDijk:83}, who introduced the
notion of local and global coherence, defining the former as a set of
``rules and conditions for the well-formed concatenation of pairs of
sentences in a linearly ordered sequence'' and specifying the latter
as contraints on the macro-structure of the narrative
\cite[see][]{Hoey:83}.  Similar ideas were also proposed
by~\citet{Longacre:79,Longacre:96}, who considered the paragraph as a
level of tagmemic grammar which was composed of multiple sentences
according to a predefined set of compositional principles.  Parallel
to this, \citet{Winter:77} presented an extensive study of different
lexical means which could connect two sentences in a text and grouped
these means into two major categories: \textsc{Matching} and
\textsc{Logical Sequence}, depending on whether the connected sentence
was elaborating on the previous content or rather adding new
information to the preceding text.

The increased interest of traditional linguistics in text-level
analysis has rapidly spurred the attention of the NLP community to the
discourse structure.  Among the first who stressed the importance of
discourse phenomena for automatic generation and understanding of
texts was \citet{Hobbs:79}, who argued that semantic links between
sentences were the most important component for building a coherent
discourse, and its lexical cohesion (\ie{} the use of coreferent and
overlapping lexical items in text) was an immediate corollary of these
relations.  Similarly to \citeauthor{Winter:77}, \citeauthor{Hobbs:79}
also proposed a set of possible inter-sentence relations, which,
however, comprised only three elements: \textsc{Elaboration},
\textsc{Parallel}, and \textsc{Contrast}.  Although this taxonomy was
obviously too small to accommodate the whole variety of different
semantic ties that could exist between sentences, it had laid the
foundation for many successful approaches to automatic discourse
processing that appeared later on.

\textbf{RST} One of the best-known such approaches, called
...
of discourse relations and expanding it to a set of 23 elements
(including new items such as \textsc{Evidence}, \textsc{Antithesis},
\textsc{Elaboration}, \textsc{Circumstance}, etc.), the authors also
introduced the distinction between nucleus-satellite and multinuclear
relations, depending on whether two clauses connected by a semantic
link were of different importance to the content of the whole text
...
\textsc{Evidence} relation as defined by~\citet{Mann:88} is shown in
...
\noindent However, the most important contribution of RST was not the
devised relation taxonomy itself but an attempt to use this taxonomy
for a comprehensive description of the overall structure of the text.
In particular, the authors defined this structure as a (projective)
constituency tree whose nodes were either elementary discourse units
or other subtrees which were connected by edges that represented the
semantic links (\ie{} relations) between these parts.

You can see an example of such rhetorical tree from the original RST
...
\cite{Wolf:05} and arbitrariness of the proposed relation set
\cite{Nicholas:94,Miltsakaki:04}.  As a result of this criticism, two
alternative approaches to automatic discourse analysis have appeared
on the scene.

\textbf{PDTB} One of these approaches---PDTB (named so after the Penn
Discourse Treebank \cite{Prasad:04})---has been developed by the
...
fully specifying the hierarchical structure of text and trying to come
up with a comprehensive set of unambiguous discourse relations, the
author simply focused on the grammatical and lexical means which could
connect two sentences.  These means were typically either coordinating
or subordinating conjunctions (\eg{} \emph{and}, \emph{because},
\emph{since}, etc.) or discourse adverbials (\eg{} \emph{however},
\emph{otherwise}, \emph{as a result}, etc.); which, according to the
original definition, had to accept two sentence arguments
(\textsc{Arg1} and \textsc{Arg2}) and express a semantic \emph{sense}
holding between these predicates.%% The choice of these senses was
explicitly
...
denoted these cases as \emph{implicit} discourse relations and asked
the annotators to label the arguments of such relations as well, but,
in addition, also specify the connective which, in their opinion, was
omitted.

Finally, if there was neither an explicit nor an implicit connective,
\citet{Prasad:04} distinguished three possibile cases: the coherence
relation was
\begin{inparaenum}[(i)]
  \item either expressed by an alternative
    lexical means which made the connective redundant
    (\textsc{AltLex}), or
  \item was achieved by referring to the same entities in both
...
different tags.  In addition, in case of reported speech, PDTB also
provided a special \textsc{Attribution} label for the text spans which
...
determined by the connective that links the arguments (\eg{} \emph{in
  fact}, \emph{while}, or \emph{if}).  At the same time, the structure
of this annotation is completely flat so that we can neither infer
which of the sentences plays the central role in the text nor see the
modification scope of the statements.
...
\textsc{Attribution} edge in RST, and its \textsc{Contrast} link is
similar to the RST's \textsc{Comparison}, but, unlike the latter
nucleus-satellite link, denotes a paratactic relationship between its
arguments.
...
would be more amenable to our sentiment experiments, we made our
choice by taking into account the following theoretical and practical
considerations: From theoretical perspective, we wanted to have a
strictly hierarchical discourse structure for the analyzed tweets so
that we could infer the polarity of the whole message by recursively
accumulating the polarity judgements from the leaves of the hierachy
up to the root.  From practical aspect, since there was no discourse
parser readily available for German, we wanted to have a maximal
assortment of such parsers for English so that we could pick one that
would be easiest to retrain on German.  Fortunately, both of these
concerns have lead us to the same solution---Rhetorical Structure
Theory, which, on the one hand, was the only formalism that explicitly
guaranteed a single root for the analyzed text, and, on the other
hand, provided a wide variety of open-source parsing systems at our
disposal \cite[\eg][]{Hernault:10,Feng:14,Ji:14,Yoshida:14,Joty:15}.
...
out all tweets which had at most one segments,\footnote{Since the
...
sets as we did in our previous work on coarse-grained sentiment
analysis.
...
three segments, while messages with more EDUs are extremely rare and
rather represent exceptions, which is also not surprising regarding
that the maximum length of a tweet is limited to only 140 characters.
Nonetheless, even with this restriction, there still are a few
microblogs with 12 and 13 discourse units.  Since it was somewhat
surprising for us to see that many segments in a single message, we
decided to look at these messages in more detail.  As it turned out,
such high number of EDUs typically arose from spurious punctuation
marks, which were carelessly used in texted messages but evidently
confused our segmenter, which had been trained on standard-language
texts (see Example~\ref{dasa:exmp:many-segments}).
...
the vast majority of the SB10k set.  Similarly, negative microblogs
again are the least represented group in both cases and account for
22\% of the former data and 16\% of the latter tweebank.
...
did not utilize the linear projection of feature values, because the
...
relations, the \textsc{Non-Contrastive} links clearly dominate both
...
\end{example}
\noindent \citet{Polanyi:06} also confirmed the important role of text
...
intensity and polarity of opinionated words.  To prove this claim,
...
via affinity edges to their immediate neighbors.  After adding two
...
aspects.  Another important contribution was made by
...
\cite{Taboada:06}.  This mishap, however, might be partially due to
...
(lexicon scores of these unigrams):
\begin{equation*}
