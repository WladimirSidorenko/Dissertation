...
\item
  ...
  considered as a sentiment in our dataset were:
...
\item
  ...
  \begin{itemize}
    ...
    \item
      ...
      which was drawn between a target and its respective sentiment in
...
sentiment, since this syntactic unit was the minimal constituent,
which simultaneously comprised both the target of the evaluation
(``Milliardeneinnahmen'', \emph{billions of revenue}) and its
respective evaluative term (``peinlich'', \emph{embarrassing}).
Furthermore, following our guidelines, the experts also included the
complete noun phrase ``diese Milliardeneinnahmen'' (\emph{these
  billions of revenue}) along with the indefinite pronoun ``diese''
(\emph{these}) into the annotated target span, since the pronoun was
syntactically dependent on the main evaluated entity --
...
\item
  ...
  In contrast to targets and sources, which only could exist in the
  presence of a sentiment, emotional expressions were always to be
  marked in text regardless of whether there was a target-oriented
  opinion or not.
  ...
  the former case, the values of these features had to reflect the
  lexical meaning of the opinionated terms without taking into account
  their context (i.e., prior polarities and intensities), whereas
  sentiments' attributes showed the compositional meaning of the whole
  opinion (i.e., contextual polarities and intensities of
  evaluations).
  ...
  to its respective sentiment when an unambiguous assignment was
  impossible;
...
\item
  ...
  annotation scheme.  Typical examples of such elements were adverbial
  modifiers, e.g., ``sehr'' (\emph{very}), ``super'' (\emph{super}),
  ``stark'' (\emph{strongly}) etc.;
...
\item
  ...
  adverbs, such as ``weniger'' (\emph{less}), ``kaum''
  (\emph{hardly}), ``fast'' (\emph{almost}) etc.
  ...
  two possible values -- \emph{medium} and \emph{strong} -- and the
  link attribute \emp{emo-expression-ref}, which had to point from the
  modifying element to the respective opinionated term being
  intensified or dampened;
