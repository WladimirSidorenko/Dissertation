...
training data:
\begin{equation}\label{snt:fgsa:eq:crf-w-regulari}
\mathbf{w} = \argmax_{\mathbf{w}}\sum_{n=1}^N\ell
\left(\mathbf{y}^{(n)}|\mathbf{x}^{(n)}\right) -
...
\todo[inline]{Add picture of hyper-parameters.}

\subsection{Feature Analysis}

In order to better estimate the effect of features on the net results
of the CRF system, we performed an ablation test, removing one group
of state attributes at a time and rechecking the performance of the
