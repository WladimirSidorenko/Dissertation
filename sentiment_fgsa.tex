...
\subsection{Definition of Sentiment, Source, and Target Spans}
...
targets and sources has not been addressed with the due attention in
the literature yet.  Researchers typically ignore this problem,
leaving its solution to the discretion of their annotators
\cite[cf.][]{Wiebe:05,Klinger:13}.

Instead of relying on intuitive decisions of our coders, we provided
an explicit rule for determining sentiments' boundaries by telling our
annotators to assign the \texttt{sentiment} tag to ``minimal complete
syntactic or discourse-level units that included both the target of an
opinion and its actual evaluation''.  According to this definition,
during annotation, our experts first had to identify evaluated objects
(targets), then find their respective evaluative expressions (usually
but not necessarily polar terms), and, finally, determine the smallest
syntactic components (noun or verb phrases) or discourse units
(clauses or sentences) where both of these entities appeared together.

An annotation example labeled in accordance with this rule is provided
below:
\begin{example}\label{snt:fgsa:exmp:sent-anno1}
  ...
  \noindent\sentiment{The new Pope is believed to be a sober, modest
    man.}
\end{example}
In this sentence, our annotators would have to label the complete
clause, since this unit was the minimal syntactic constituent which
included both the target of an evaluation---``der neue Papst''
(\textit{the new pope})---and the evaluation itself--- ``bescheidener,
zur\"uckgenommener Typ'' (\textit{a sober, modest man}).

The same principles of minimality and completeness were enforced on
sources and targets of opinions too, whose main elements (nouns or
verbs) had to be labeled along with their dependent attributes.
