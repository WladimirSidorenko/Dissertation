...
\citet{Esuli:06c}, \citet{Rao:09}, and \citet{Hassan:10}, applying
...
the figures, such PoS-classes as adverbs and adjective satellites are
missing completely in the German resource.  The reason for this is
that the form (and meaning) of most German adverbs typically coincides
with that of the adjectives; therefore, both categories are treated
the same way, being represented as adjectival synsets.
...
that are connecting different part-of-speech classes: As can be seen
from the figures, the strongest inter-PoS connections in
\textsc{GermaNet} are the pertainym links between adjectives and nouns
and participle bounds between adjectives and verbs.  The interlinks
between nouns and verbs, however, are both much fewer in number and
more diverse in their nature.  This situation is different for
\textsc{WordNet} where the prevailing majority of inter-part-of-speech
connections are the \texttt{related\_to} relations (especially between
nouns, adjective satellites, verbs, and verbs and adjectives) and
\texttt{derived\_from} (especially between adverbs and adjectives with
their satellites).  The links between adjectives and nouns are mixed
though, featuring both \texttt{related\_to} and \texttt{derived\_from}
connections.  As we should see later, these links are crucial for
breaking part-of-speech dependencies of seed sets in the cases when
all seeds belong to the same PoS class.



Relations: \citet{Kamps:04} direct antonymy, similarity, derived-from,
pertains-to, attribute, and also-see ,
...
\begin{table}[h]
  \begin{center}
    ...
    \caption{Evaluation of dictionary-based approaches.\\ {\small (HL
        ...
        \citet{Rao:09}, AR -- \citet{Hassan:10})}}
    \label{snt-lex:tbl:gsl-res-full}
  \end{center}
\end{table}
