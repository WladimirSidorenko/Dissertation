...
matrices, finding an optimal update rate (i.e. the length of the
update step made in the direction of the gradient) might pose
considerable difficulties, leading either to a prohibitively large
training time or to a complete divergence of the trained model.
...
RmsProp~\cite{Tieleman:12} etc.  In our subsequent experiments, we
will rely on the last option---the RmsProp optimization introduced
by~\citet{Tieleman:12}---as we found this algorithm leading to a
faster convergence and a superior classification quality.
