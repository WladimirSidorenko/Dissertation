...
work~\citet{Severyn:15}, whose feed-forward DL-based system ranked
...
success, were
\begin{inparaenum}[(i)]
\item that the word2vec vectors were used only for initialization, but
  were then modified along with other model's paramaters
  (convolutional filters and dense matrix) during training, and
\item that \citeauthor{Severyn:15} enhanced the original SemEval
  training set with an additional, much larger noisily labeled corpus.
\end{inparaenum}
These improvement allowed the authors to establish a new state of the
art on the phrase-level polarity prediction task with a macro-averaged
\F-score of 0.8479 and also obtain a competitive result of 0.6459~\F
