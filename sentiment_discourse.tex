...
with the help of the lexicon-based attention classifier introduced in
the previous chapter.  This left us with a total of 4,771 microblogs
...
most of tweets in both corpora typically comprise two or three
...
surprising to see that many discourse units in a single message, we
decided to look at these tweets in more detail.  As it turned out,
such high number of EDUs typically arises from spurious punctuation
marks, which are carelessly used in texted messages but apparently
confuse our segmenter, which has been trained on standard-language
data.  You can see an example of such carelessly composed tweets with
13 EDUs in~\ref{dasa:exmp:many-segments}.
...
Another trend which we also can observe in the data is that the
distribution of polarity classes in messages with multiple EDUs
...
22\% of the former collection and 16\% of the latter tweebank.

To obtain RST trees for segmented messages, we retrained the DPLP
discourse parser of~\citet{Ji:14} on the PCC~2.0 dataset
\cite{Stede:14}, after mapping all discourse relations of this corpus
to the binary set $\{$\textsc{Contrastive},
...
we did not utilize a linear projection of the feature values, because
the released parser code did not include this procedure either.  In
part due to these modifications, but mostly because of the specifics
of the German language, the results of our retrained model are
...
71.13, and 61.63~\F{} on the English dataset.\footnote{Following
  \citet{Ji:14}, we use the span-based evaluation metric introduced
  by~\citet{Marcu:00}.}

Figure~\ref{dasa:fig:twitter-rst-tree} shows an example of an
automatically induced RST tree for a Twitter message.  As we can see
from the analysis, the adapted parser can correctly distinguish
between contrastive and non-contrastive relations, but apparently
struggles with the disambiguation of the nuclearity status of EDUs,
assigning the highest significance to the initial discource segment
(``Mooooiiinn.'' [\emph{Hellloooo!}]), which is merely a greeting, and
weighing the third discourse unit (``Aber Kaffee!'' [\emph{But
    coffee!}]) more than the second one (``Gegen solche N\"achte hilft
die beste Kur nicht.'' [\emph{Even the best cure won't help against
    such nights.}]), although traditional RST would consider both
...
Regarding the distribution of the two remained coarse-grained
discourse relations, as we can see from
Figure~\ref{dasa:fig:relation-distribution}, the
\textsc{Non-Contrastive} ties clearly dominate both datasets,
accounting for 98\% of all discourse links in PotTS and SB10k.  By
comparison, the precentage of these relations in PCC~2.0 run up to
90\% and also represented the prevailing majority of all semantic
links in this corpus.
