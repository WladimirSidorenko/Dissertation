...
\item\emph{least-squares embeddings}, in which we again learned
  ...
  respective matrix of word2vec embeddings, and $\lVert\cdot\rVert_F$
...
preparing the data, we pre-processed all corpus tweets using the
...
\item replaced syntactically integrated Twitter-specific phenomena
  ...
  tokens representing their semantic class (e.g., ``\%Username'' for
  @-mentions, and ``\%URI'' for hyperlinks);
\item removed these phenomena if they were syntactically independent
  and did not play an important role for the succeeding sentiment
  analysis (e.g., we stripped off all retweets and hyperlinks appearing
...
\item substituted all emoticons with unified placeholders representing
  their polarity classes (e.g., \smiley{} $\rightarrow$
  ``\%PosSmiley'', \frownie{} $\rightarrow$ ``\%NegSmiley'',
  \texttt{:-O} $\rightarrow$ ``\%Smiley'' etc.);
...
\item and, finally, restored some common forms of misspellings (e.g.,
  ...
  $\rightarrow$ ``Tag'' (\emph{day}) etc.).
\end{itemize}

Even though these transformations were supposed to improve the
grammaticality of the sentences, an opposite effect of the
normalization could be the loss of (potentially valuable) surface
features.  In order to check, which of these implications was
stronger, we repeated the evaluation once again, turning the
...
\begin{table*}
  \begin{center}
    ...
    \begin{tabular}{p{0.162\columnwidth} % first columm
      ...
      CRF & 0.376 & 0.319 & 0.345 & %
       0.298 & 0.33 & 0.313 & %
       0.293 & 0.231 & 0.258 & 0.305\\
    ...
    \end{tabular}
    \egroup
    \caption{Results of fine-grained sentiment analysis with and
...
Table~\ref{snt-fgsa:tbl:normalization},

\subsection{Effect of Sentiment Lexicons}
...
\begin{table*}
  \begin{center}
    ...
    \begin{tabular}{p{0.162\columnwidth} % first columm
      ...
      \multicolumn{11}{c}{\cellcolor{cellcolor}No Lexicon}\\
      ...
      \multicolumn{11}{c}{\centering\cellcolor{cellcolor}
      \specialcell{Zurich Polarity Lexicon\\\cite{Clematide:10}}}\\
      ...
      \multicolumn{11}{c}{\cellcolor{cellcolor}\citet{Blair-Goldensohn:08}}\\
      ...
      \multicolumn{11}{c}{\cellcolor{cellcolor}\citet{Takamura:05}}\\
      ...
      \multicolumn{11}{c}{\cellcolor{cellcolor}Linear Projection}\\
