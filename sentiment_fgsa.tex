...
Aspect-based sentiment analysis (ABSA) deals with identification of
subjective evaluative opinions (\emph{sentiments}), holders of these
opinions (\emph{sources}), and their respective evaluated entities
(\emph{targets}) in text.  Since an accurate automatic prediction of
these elements would allow us to track the public's attitude towards
literally any object (\eg{} a product, a service, or a political
decision), ABSA is traditionally considered as one of the most
attractive, necessary, but, unfortunately, also challenging objectives
in the opinion-mining field.

Researchers usually interpret this objective as a sequence labeling
(SL) task, and address it with one of the two most popular SL
...
probabilistic graphical model, which relies on an extensive set of
hand-crafted features, whereas the latter methods use a recursive
computational loop and learn their feature representations completely
automatically.  In this chapter, we are going to evaluate each of
these solutions in detail in order to find out which of these
algorithms is better suited for the domain of German Twitter.  But
before we proceed with our experiments, we should first briefly
discuss evaluation metrics that we will use to estimate the quality of
these systems.
...
Because aspect-based sentiment analysis operates on \emph{spans} of
sentiment labels, which typically consist of multiple contiguous tags,
we cannot straightforwardly apply metrics that are used to evaluate
the classification of single instances to this objective, as it is
unclear which units should be measured---single tokens or complete
spans---and how partial matches should be counted in the latter case.
One possibility to estimate the quality of ABSA prediction is to
compute precision, recall, and \F{}-scores of predicted spans by using
binary-overlap or exact-match metrics~\cite[see][]{Choi:06,Breck:07}.
The first method considers an automatically labeled span as correct if
it has at least one token in common with a labeled element from the
gold annotation.  The second metric only counts an automatic span as
true positive if its boundaries are absolutely identical with the span
boundaries annotated by the human expert.  Unfortunately, both of
these approaches are somewhat problematic: While binary overlap might
be overly optimistic, always assigning perfect scores to automatic
spans that cover the whole sentence; exact match might, vice versa, be
too drastic, considering the whole assignment as false if only one
(possibly irrelevant) token is classified incorrectly.
...
the sets of manually and automatically tagged spans ($\mathcal{S}$ and
...
In order to evaluate CRFs and RNNs on our dataset, we split our corpus
...
support for the character offsets of the annotated tokens and because
automatically tokenized data could disagree with the original corpus
tokenization, we aligned the manual annotation with the automatically
split words by using the Needleman-Wunsch
...
\item and, finally, the \emph{avoidance of the label bias problem},
  ...
  \begin{example}[Label Bias Problem]
    ...
    the correct labeling ($KON-ADV-NN-VA$) is equal to:
    \begin{align*}
  ...
  \end{example}
\end{enumerate}
\textbf{Training.}\quad{}CRFs have these useful properties due to a
neatly formulated objective function in which they seek to optimize
the global log-likelihood of gold labels $\mathbf{Y}$ conditioned on
training data $\mathbf{X}$.  In particular, given a set of training
instances $\mathcal{D} = \{(\mathbf{x}^{(n)},
\mathbf{y}^{(n)})\}_{n=1}^N$, where $\mathbf{x}^{(n)}$ stands for the
input variables of the $n$-th instance, and $\mathbf{y}^{(n)}$ denotes
its respective gold labels, CRFs try to find feature coefficients
$\mathbf{w}$ that maximize the log-probabilities $\ell$ of
$\mathbf{y}^{(i)}$ given $\mathbf{x}^{(i)}$ over the whole corpus:
...
the same expectation under model's parameters $\mathbf{w}$.  In other
...
The marginal probabilities of these feature, which are required for
...
\noindent\textbf{Inference.}\quad{}Once optimal feature weights are
found, one can unproblematically compute the most likely label
assignment for a new instance by using the Viterbi
algorithm~\cite{Viterbi:67}, which effectively corresponds to the
forward pass of the FB method with the summation over the alternative
preceding states replaced by the maximum operator (hence the other
name for this algorithm, ``max-product'').

\noindent\textbf{Features.}\quad{}A crucial component that accounts
for a huge part of the success (or failure) of CRFs is features that
are provided to this classifier as input.
...
reflects various properties of the input sequence, such as whether the
current token is capitalized or whether it begins with a specific
prefix or ends with a certain suffix.  This type of features commonly
accounts for the overwhelming majority of all attributes used in CRFs.

As state attributes in our experiments, we used the following types of
predicates, which, for simplicity, are listed in groups:
\begin{itemize}
\item\emph{formal}, which included the initial three characters of
  ...
  three characters, and the general spelling class of that word (\eg{}
...
\item and, finally, \emph{syntactic} features, which reflected the
  ...
  parent.  Furthermore, we used two binary attributes that showed
  whether the previous token in the sentence was the parent or child
  of the current word. Apart from that we devised two more features,
  one of which encoded the dependency relation of the previous token
  in the sentence to its parent + the dependency relation of the
  current token to its ancestor, and another one reflected the
  dependency link of the next token + dependency relation of the
  current token to its parent.
\end{itemize}

In addition to the above attributes, we also introduced a set of
complex \emph{lexico-syntactic} features, which simultaneously
reflected several semantic and syntactic traits.  These were:
\begin{itemize}
...
the given set of features, CRF model can perfectly well fit the
training data, achieving a macro-averaged \F-score of~0.904.  These
parameters, however, only partially generalize to unseen tweets,
leading to notably lower \F-results on the test corpus (0.287), which
indicates an overfitting of the training data, although we did our
best to combat this effect.\footnote{To mitigate overfitting, we used
  early stopping and higher regularization parameters.}  Another
notable tendency, which can be observed both on the training and test
sets, is that the recall of the CRF system is generally lower than its
precision.  This again can be attributed to the high variance of the
classifier.
...
\begin{table*}
  \begin{center}
    ...
    \end{tabular}
    \egroup{}
    \caption{Results of aspect-based sentiment analysis with the
...
To see whether the primary reason for this overfitting was suboptimal
features that we provided to the system as input, we performed an
ablation test, removing one group of state attributes at a time and
...
attributes.  A different situation is observed for \markable{source}s
and \markable{target}s.  In the former case, removing formal,
...
the statistics in Table~\ref{fgsa:tbl:ablation}, three of the five
highest-ranked state attributes (``meiste'' [\emph{most}] and
``rettungsschirme'' [\emph{bailout}], which typically indicate a
\markable{target}, and ``urlauberin'' [\emph{holiday}], which
frequently correlates with \markable{sentiment}s) are complex features
reflecting the lemma of the syntactic parent.  Another common group of
features is lemma and form of the current token: here, we again
encounter the word ``meisten'' (\emph{most}), which, however,
indicates the absence of any sentiment entities this time, and we also
can see two other attributes (``anfechten'' [\emph{doubt}] and
``bezeichnete'' [\emph{called}]) that represent the so-called
\emph{direct speech events} and correlate with \markable{sentiment}
tags; the remaining feature (``thomasopperman'') is a person name,
which frequently appears as sentiment's \markable{target} in our
corpus.

An interesting pattern can be observed for transition features: As we
can see from the results, top three of these attributes indicate a
strong belief that an objective token is very unlikely to be followed
by a \markable{target}, \markable{source}, or \markable{sentiment} tag
(hence, the high negative weights of transitions emanating from
\textsc{NON}).  It is, however, quite common that a \textsc{NON} tag
will precede another \textsc{NON} instance (as we can see from line 5
of the table).  Other transitions also mainly reflect plausible
regularities: It is, for instance, uncommon that a \markable{target}
of an opinion will appear immediately before a source
(\textsc{TRG}$\rightarrow$\textsc{SRC} $= -2.99$); in the same vein,
it is fairly improbable that the SRC tag will precede a
\markable{target} element (\textsc{SRC}$\rightarrow$\textsc{TRG} $=
...
In this case, the classifier erroneously overlooked a negative
...
(none) tags to all tokens of that tweet instead.  As it turns out,
despite this incorrect assignment, the state potentials of the smiley
still achieve their highest scores with the correct SNT
(\markable{sentiment}) tag.  Moreover, the state scores of the word
``Plakate'' (\emph{posters}) also reach their maximum value (0.13 in
the logarithmic domain) with the correct label TRG
(\markable{target}).  Unfortunately, these good guesses for single
tokens are overruled by the extremely high score for the NON tag
(6.515) that is assigned to the first word of this message
(``\"uberall'' [\emph{everywhere}]) and is reinforced by the
transition features, which prefer NON labels to be followed other NON
tags.

This kind of mistakes is by far the most common type of errors that we
observed on the development set, followed by spans with different
...
fixed-size hidden vector $\vec{h}$, which is recurrently updated while
processing an input sequence $\mathbf{x}$ and is supposed to encode
the meaning of that sequence seen so far.  The general form of this
vector at input state $t$ is usually defined as:
\begin{align*}
...
\textbf{LSTM.}\quad{}A fundamental problem that arises from the above
definition is that the gradients of model's parameters rapidly vanish
to zero or explode to infinity (depending on whether the absolute
values of $\vec{h}$ are less or greater than one) as the length of the
input sequence increases.  In order to solve this issue,
\citet{Hochreiter:97} proposed the long short-term memory mechanism
(LSTM), in which they explicitly incorporate the goal of keeping the
gradients within an appropriate range.  In particular, given an input
sequence $\mathbf{x}$, they introduce a special \emph{activation unit}
$\vec{i}^{(t)}$:
\begin{align*}
...
$\vec{b_i}$ represent model's parameters; and $\mathbf{x}^{(t)}$ and
$\vec{h}^{(t-1)}$ stand for the input and previous hidden states
respectively.  In addition to that, the authors estimate a dedicated
\emph{forget gate}~$\vec{f}^{(t)}$:
\begin{align*}
...
\end{align*}
they estimate the \emph{final update}~$\vec{c}^{(t)}$ by taking a
...
\end{align*}
from which, they finally compute the output vector $\vec{o}^{(t)}$ and
the new value of the hidden state $\vec{h}^{(t)}$:
\begin{align*}
...
\textbf{GRU.}\quad{}Despite their enormous
...
\end{align*}
With the help of these gates, they estimate the candidate activation
$\widetilde{c}^{(t)}$ as:
\begin{align*}
  \widetilde{c}^{(t)} &= tanh\left(W_c\cdot \mathbf{x}^{(t)} + U_c
  \cdot \left(\vec{f}^{(t)} \odot \vec{h}^{(t-1)}\right)  + \vec{b}_c\right),
\end{align*}
and compute the hidden state $\vec{h}^{(t)}$ as:
\begin{align*}
...
represent label probabilities (because the elements of these vectors
can also be negative, and their sum does not necessarily run up to
one), and, moreover, because the size of our tagset (four tags:
\textsc{SNT}, \textsc{SRC}, \textsc{TRG}, and \textsc{NON}) is
obviously too small for the size of the hidden unit, we set the
dimensionality of the intermediate RNN vectors to 100, and apply a
linear transformation matrix $O \in \mathbb{R}^{4 \times 100}$ to the
final output of the recursion loop, computing the softmax of their dot
product:
\begin{align*}
...
\textbf{Training.}\quad{}A neat property of LSTM and GRU is that the
final equation, which is obtained after unrolling the recurrence loop,
is differentiable with respect to all of its parameters, and can
therefore be optimized with standard gradient update techniques.
...
training times (if the steps are too small) or complete divergence of
the trained model (if the steps are too large).
...
of these options---RMSProp~\citet{Tieleman:12}---as this algorithm
...
results were the initial values of models' parameters.  As shown
...
Following the recommended practices~\cite{Saxe:13}, we used orthogonal
...
\begin{table*}
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of aspect-based sentiment analysis with recurrent
      neural networks}
...
\begin{table*}
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of aspect-based sentiment analysis with different
      word embeddings}
...
After estimating the results of popular ABSA approaches with their
...
As the first factor that could affect the quality of automatic ABSA
...
\begin{table*}[hbt!]
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of aspect-based analysis with broad and narrow
...
\begin{table*}[hbt!]
  \begin{center}
    ...
    \egroup{}
    \caption[Results of aspect-based sentiment analysis with different
      CRF topologies]{Results of aspect-based sentiment analysis with
...
In order to see whether the same tendencies hold for deep-learning
methods, we also implemented higher-order and tree-structured
extensions of LSTM and GRU.  In the former case, we passed a
concatenation of $n$ preceding $\vec{h}$ vectors (where $n$ is the
order of the model) as input to the recurrence loop.  In the
...
domain are standard-language news texts.
...
\begin{table*}
  \begin{center}
    ...
    \egroup{}
    \caption[Results of aspect-based sentiment analysis with different
    neural network topologies]{Results of aspect-based sentiment
...
the ABSA results, we repeated the evaluation once again, turning the
...
\begin{table*}[bht!]
  \begin{center}
    ...
    \end{tabular}\egroup{}
    \caption{Results of aspect-based sentiment analysis with (w) and
...
%% social impact and big economic importance, ABSA has always been an
...
%% A recurrent approach to the ABSA problem was proposed
...
%% into the abilities of modern ABSA approaches, the importance of their
...
%% described ABSA methods.\footnote{Unfortunately, we were not able to do
