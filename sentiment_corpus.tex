...
of German microblogs posted in April 2013.  This tracking procedure
gave us a total of 27.4 M messages with the M\"unster corpus and
Scheffler's collection being by far the most prolific sources of the
data.
...
were also indirectly more likely to contain subjective opinions.
...
\item
  ...
  \begin{itemize}
    ...
    \item
      ...
      had to help us disambiguate the assignment of evaluated objects
      to their pertaining appraisals when this association was not
...
In this sentence, our annotators labeled the complete clause as
...
\item
  ...
  whereas sentiment's attributes showed the compositional meaning of
  the whole opinion (i.e., contextual polarity and intensity of the
  evaluation).
...
\item
  ...
  adverbial modifiers, e.g., ``sehr'' (\emph{very}), ``super''
...
\item
  ...
  adverbs such as ``weniger'' (\emph{less}), ``kaum'' (\emph{hardly}),
...
\item
  the final element -- \textbf{negations} -- was defined as a
...
A complete annotation example with both sentiment- and
polar-term-level annotations is shown below:
\begin{example}\label{snt:exmp:sent-anno2}
  ...
  \end{tikzpicture}
\end{example}
In this case, again, the complete sentence is annotated as sentiment,
...
(\emph{not}), on the other hand, only comprise single words without
...
that topic were distributed in equal proportions.
...

{
  ...
  \item\texttt{annotator-X/}, where X stands for the annotator's id.
    ...
    specify the paths to the annotation directory, tokenization data,
    appearance settings etc.) and the subfolder \texttt{markables/},
    which comprises the actual annotations;
...
\end{equation*}
where $T$ stands for the total number of tokens, $A_1$ and $A_2$ are
the numbers of tokens annotated with the given class by the first and
...
and second annotation would then be equal to $7$ and $6$
respectively), and we only calculated the actual number of tokens with
matching labels when computing the $M$ scores (i.e., both $M_1$ and
$M_2$ would come to $6$ with this metric).  The exact values of the
binary and proportional kappa for Example~\ref{example:snt:iaa} would
then correspondingly run up to 1.0 and 0.0, since the binary metric
would consider both annotations as perfectly matched because every
labeled sentiment instance agreed on at least one token, whereas the
proportional measure would have the opposite view, stating that the
observed agreement $p_o$ was the same as the agreement by chance $p_c$
...
adjusted $\kappa$-metrics are shown in Table~\ref{tbl:snt:agrmnt}.  As can
