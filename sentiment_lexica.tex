...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \State\vars{O}$_{subj}\gets\mu_{\mathcal{S}} -
    \mu_{\mathcal{O}}$;\Comment{Compute the origin of the
      subjectivity axis.}
    ...
    \State\vars{O}$_{pol}\gets\mu_{\mathcal{P}} -
    \mu_{\mathcal{N}}$;\Comment{Compute the origin of the polarity
      axis.}
...
respect to the projection line~$\vec{b}$:
...
gradient, we then trivially optimize the line~$\vec{b}$ using gradient
...
\begin{figure}
  \centering
  \mbox{\includegraphics[height=15em]{img/sentilex_L_M}}
  \caption[Visualization of the linear projection method in a
