...
key factors:
\begin{inparaenum}[(i)]
\item by scores predicted by the base sentiment system for messages
  and single EDUs and
\item by the structure of RST trees constructed for these messages.
\end{inparaenum}
In order to estimate the effect of these factors in more detail, we
...
To assess the impact of the former factor (base sentiment classifier),
we replaced all polarity scores produced by the LBA system with
respective values returned by the best lexicon- and ML-based CGSA
methods (the systems of \citeauthor{Hu:04} [\citeyear{Hu:04}] and
\citeauthor{Mohammad:13} [\citeyear{Mohammad:13}]) and retrained all
DASA approaches on the updated data, subsequently evaluating them on
the PotTS and SB10k test sets.  The results of this evaluation are
...
As we can see from the first figure, our initially chosen LBA system
is indeed a more reliable base predictor for almost all
discourse-aware sentiment approaches on the PotTS corpus.  A few
exceptions to this general rule are the macro-averaged \F{}-score of
the \textsc{Last} baseline, which surprisingly improves in combination
with the lexicon-based approach, and the micro-\F{} of \textsc{RDP}
and \textsc{Last}, which attain their best results (0.713 and 0.582)
in conjunction with the SVM classifier of \citet{Mohammad:13}.
...
\textsc{Root} systems; but approach of \citet{Mohammad:13} improves
the results of \textsc{LCRF}, \textsc{LMCRF}, \textsc{RDP}, and
...
solution performs much weaker than SVM and LBA: the highest macro- and
micro-averaged \F{}-scores achieved with this system run up to 0.422
(\textsc{RDP}) and 0.625 (\textsc{Last}) respectively.  The most
disappointing result for us, however, is that the LMCRF system
completely fails to predict any polar class except \textsc{Neutral} on
the SB10k test set when trained with the scores produced by predictor.
Likewise, LCRF can hardly outperform any other competitor, reaching
only 0.239 macro-\F{}.

\subsection{Parsing Quality and Relation Scheme}

Another factor which could significantly influence the results of
discourse-aware sentiment systems was the quality of the automatic RST
parsing and the set of discourse relations distinguished by the parser
system.  Although improving the results of DPLP let alone manually
annotating the complete PotTS and SB10k datasets was beyond the scope
of this dissertation, we decided to check whether using hand-crafted
RST trees in the test input could improve the scores of our tested
DASA methods.  For this purpose, we asked a student assistant to
segment messages and annotate discourse trees for 88\% of the PotTS
test data\footnote{Unfortunately, we could not annotate the whole test
  set due to time limitations of the student.}  and evaluated all
trained systems on these trees.
\begin{table}[htb]
  \begin{center}
    \bgroup \setlength\tabcolsep{0.1\tabcolsep}\scriptsize
    \begin{tabular}{p{0.162\columnwidth} % first columm
