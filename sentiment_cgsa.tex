...
To estimate the quality of a feature, we use In- formation Gain (IG)
...
parameter repre- senting an acceptable margin of performance dif-
...
be a lit- tle worse than feature B, but within an acceptable
...
1Grams will always sur- vive the subsumption process because they
...
classi- fier that uses all three types of features. Next, we created
analogous 1+2Gram, 1+EP, and 1+2+EP classifiers but applied the
subsumption hierar- chy first to eliminate unnecessary features be-
fore training the classifier. We experimented with three delta values
for the subsumption process: $\delta$=.0005, .001, and .002.

Figures 7, 8, and 9 show the results. The sub- sumption process
...
are higher than the ac- curacy values produced without subsumption
...
best classi- fier trained after subsumption (but without feature
selection). Furthermore, the combination of sub- sumption plus feature
...
curacy results were achieved by performing sub- sumption prior to
feature selection. The best accu- racy results are 99.0\% on the OP
