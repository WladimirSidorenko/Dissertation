...
\begin{equation*}
  \theta^* = \argmax_{\theta}\ell(\theta) = \sum_{i=1}^{N}\log\left(p\left(\mathbf{y}^{(i)}\vert\mathbf{x}^{(i)}; \theta\right)\right),\label{dasa:eq:crf-objective}
\end{equation*}
...
\begin{equation*}
\mathbf{\theta}^* =
\argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\log\left(p\left(\mathbf{y}_o^{(i)}\vert\mathbf{x}^{(i)};
\theta\right)\right).
\end{equation*}
...
\begin{align}
  \begin{split}
  \mathbf{\theta}^* &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\log\left(p\left(\mathbf{y}^{(i)}\right)\right) - \log\left(p\left(\mathbf{y}^{'(i)}\right)\right)\\
  &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\log\left(\exp{\mathbf{\theta}^{\top}\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)})}\right) - \log\left(\exp{\mathbf{\theta}^{\top}\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})}\right)\\
  &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\mathbf{\theta}^{\top}\left(\mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{(i)}) - \mathbf{f}(\mathbf{x}^{(i)},\mathbf{y}^{'(i)})\right),\label{dasa:eq:hcrf-objective}
  \end{split}
\end{align}
...
\begin{align}
  \begin{split}
  \mathbf{\theta}^* &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}\frac{p(\mathbf{y}^{(i)})}{p(\mathbf{y}^{'(i)})}\\
  &= \argmax_{\mathbf{\theta}}\sum_{i=1}^{N}
  \frac{\sum_{\mathbf{y}_h^{(i)}}\exp\left(\sum_{t=0}^{T_i}\left[%
    \sum_v\mathbf{\theta}_vf_v\left(\mathbf{x}^{(i)}_t,\mathbf{y}^{(i)}_{t}\right)
    + \sum_{c\in
      ch(t)}\sum_e\mathbf{\theta}_ef_e\left(\mathbf{y}^{(i)}_{t},
...
single parameter $\theta_v$ as:
\begin{align*}
  \frac{\partial{}a}{\partial\theta_v} &= a\sum_{t=0}^{T_i}f_v\left(\mathbf{x}^{(i)}_t,\mathbf{y}^{(i)}_{t}\right)\propto\mathbb{E}_{\mathbf{y}^{(i)}}f_v,\\
  \frac{\partial{}b}{\partial\theta_v} &= b\sum_{t=0}^{T_i}f_v\left(\mathbf{x}^{(i)}_t,\mathbf{y}^{(i)}_{t}\right)\propto\mathbb{E}_{\mathbf{y}^{'(i)}}f_v;
\end{align*}
and analogously obtain:
\begin{align*}
  \frac{\partial{}a}{\partial\theta_e} &= a\sum_{t=0}^{T_i}\sum_{c\in ch(t)}f_e\left(\mathbf{y}^{(i)}_{t}, \mathbf{y}^{(i)}_{c}\right)\propto\mathbb{E}_{\mathbf{y}^{(i)}}f_e,\\
  \frac{\partial{}b}{\partial\theta_e} &= b\sum_{t=0}^{T_i}\sum_{c\in ch(t)}f_e\left(\mathbf{y}^{'(i)}_{t}, \mathbf{y}^{'(i)}_{c}\right)\propto\mathbb{E}_{\mathbf{y}^{'(i)}}f_e.
\end{align*}
With the help of these expressions, we can easily compute the gradient
of the objective function w.r.t. $\mathbf{\theta}$ by observing that:
\begin{align*}
  \nabla_{\mathbf{\theta}} &=
  \sum_{i=1}^{N}\frac{\sum_{\mathbf{y}_h^{(i)}}\nabla_{\mathbf{\theta}}a\sum_{\mathbf{y}_h^{(i)}}b
    -
    \sum_{\mathbf{y}_h^{(i)}}a\sum_{\mathbf{y}_h^{(i)}}\nabla_{\mathbf{\theta}}b}{\left(\sum_{\mathbf{y}_h^{(i)}}b\right)^2}.
\end{align*}
