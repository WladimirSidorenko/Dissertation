...
the PotTS data, which is mostly due to a lower recall of the negative
class.

\subsection{Error Analysis}

\begin{example}[LCRF Error]\label{snt:dasa:exmp:lcrf-error}
\end{example}

\begin{example}[LMCRF Error]\label{snt:dasa:exmp:lmcrf-error}
\end{example}

\begin{example}[RDP Error]\label{snt:dasa:exmp:rdp-error}
\end{example}

\begin{example}[Error Made by the \textsc{Last} System]\label{snt:dasa:exmp:last-error}
\end{example}

\begin{example}[Error Made by the \textsc{Root} System]\label{snt:dasa:exmp:root-error}
\end{example}
...
%%   \centering
%%   \includegraphics[width=\linewidth]{img/dasa_potts_edu_distribution.png}
%%   \caption{PotTS}\label{dasa:fig:relation-scheme-effect}
%% \end{figure*}
...
In the most extreme cases (\citeauthor{Bhatia:15} versus PCC), these
gains can even run up to seven percent. However, with respect to other
metrics (first of all, span and nuclearity \F{}), these gaps are not
that big and in some cases might even be in favor of the richer set
(\eg{} nuclearity \F{} for PCC).
...
with the R2N2 system, which reaches its best results with the set
by~\citet{Zhou:11} (0.572 macro-\F{} and 0.719 micro=\F{}).
