...
Penn Discourse Treebank \cite{Prasad:04})---was developed by the
discourse research group of the University of Pennsylvania
\cite{Miltsakaki:04,Miltsakaki:04a,Prasad:08}, and, at its core,
represents an \emph{underspecification} of RST: Instead of fully
specifying the hierarchical structure of the text and trying to come
up with a comprehensive set of unambiguous discourse relations, the
author proceeded bottom-up, and focused only on the elements which
...
\emph{however}, \emph{otherwise}, \emph{as a result}, etc.).  Each
connective was assumed to accept two sentence arguments: \textsc{Arg1}
and \textsc{Arg2}; and have one of the predefined semantic
\emph{senses}, whose choice was explicitly restricted for each
connective (for example, the set of possible senses for
\emph{nonetheless} included \textsc{Comparison}, \textsc{Conjunction},
\textsc{Contra-Expectation}, and \textsc{Contrast}).

Apart from the explicit mentions of connectives, \cite{Prasad:04} also
allowed for the situations where the connective was missing from the
text but could be easily inferred by the reader.  The authors denoted
these situations as \textsc{Implicit} discourse relations and asked
...
serve as a connective in that case.

Finally, if there was neither an explicit connective nor an implicit
one, which could be deduced, \cite{Prasad:04} distinguished three
different possibilities:
\begin{inparaenum}
  \item either the coherence was expressed by an alternative lexical
    means which made the connective redundant (\textsc{AltLex}), or
...
Wall Street Journal, using which we previously exemplified the RST
tree, but now annotated with PDTB-style relations.  As we can see from
this analysis, the PDTB approach is indeed more flexible in comparison
with RST in the sense that it allows the discourse units (arguments)
to be overlapping, disjoint, or even embedded into other ones.  At the
same time, the assignment of sense relations is also more
straightforward as it mainly goes by the connective that connects the
arguments (\emph{in fact}, \emph{while}, or \emph{if}).  On the other
hand, the structure of this annotation is completely flat so that we
cannot infer from it which of the sentences is more important or plays
...
\todo[inline]{SDRT: \citet{Lascarides:01}, \citet{Asher:05}}
