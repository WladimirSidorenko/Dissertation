...
\begin{table*}
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of the fine-grained sentiment analysis with the
...
\item \emph{the ability of these systems to learn optimal feature
  ...
  or CRFs where all features need to be defined by the user; and
...
$\vec{b_i}$ represent the optimized model's metrices and vector; and
...
case of LSTM, and $\vec{h}^{(t)}$ in the case of GRU), we estimated
...
of~\citet{Tieleman:12}---as this algorithm typically showed both a
...
As shown by~\citet{He:15}, an inappropriate initialization of a neural
...
applied a uniform He sampling~\cite{He:15} for setting the initial
values of the bias vectors.
...
objective function $L$:\footnote{Since most of the tokens in the
  ...
  function only penalized incorrectly predicted labels or correct tags
  whose probability was insufficiently high (less than $c$), but did
...
\begin{table*}
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of the fine-grained sentiment analysis with recurrent
      neural networks.}
...
approach is precision of sentiment spans, which, however, is more than
compensated for by a much higher recall.  Moreover, the overfitting
effect is significantly less pronounced than in the CRF case (where
the \F scores on training and test data differed by a factor of
three).  Nonetheless, both RNN systems achieve lower results than the
...
expressions found in the corpus (which we term a \emph{narrow}
...
\begin{table*}[hbt!]
  \begin{center}
    ...
    \end{tabular}
    \egroup
    \caption{Results of the fine-grained analysis with the broad and
...
hand, such short spans lead to disrupted label chains for
...
effect of these features is rather negative, since, in contrast to
polar terms, being a holder or target of an opinion is not an inherent
property of a lexical expression, but arises from the context which
...
\begin{table}[hbt]
  \begin{center}
    ...
    \caption[Results of feature ablation tests for CRF model.]{
      Results of feature ablation tests for CRF
      model.\\{\small\itshape (negative changes w.r.t. the original
        scores on the dev set are shown in
