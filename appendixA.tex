...
entities, or events.  Your goal is to annotate both the text spans
denoting the opinions (\emph{sentiments}) and the text spans
signifying the evaluated entities and events (\emph{sentiment
  targets}).  In addition to that, you also have to label opinions'
holders (\emph{sentiment sources}) and lexical elements that might
...
\begin{itemize}
  ...
  \item\emph{intensifiers} and \emph{diminishers} (or
    ...
    or decrease the evaluative sense of an emotional expression.
    Examples of intensifiers are words like \emph{sehr} (\emph{very}),
...
on one of the \texttt{*.mmax} files in that folder.
...
\item it has to be \textbf{polar}, \ie{} it must always reflect either
  ...
  non-evaluative statements such as \textit{Ich glaube, er wird heute
    fr\"uher kommen} (\textit{I think he will be earlier today}) do
  not represent \texttt{sentiment}s;
...
\item a sentiment has to be \textbf{evaluative}, which means that it
  ...
  anything in particular, but simply express general mood.
\end{itemize}
...
express a relative subjective judgment, \ie{} an object is regarded to
be better or worse than another one, but we usually do not know
whether the author actually likes or dislikes any of them.  To
distinguish such cases, we have introduced a special
\texttt{comparison} value for the \texttt{polarity} attribute of this
element, which you should use to distinguish such cases.

You should not label as sentiments polar opinions whose truth status
is unknown.  These are sentences like \textit{Ich wei\ss{} nicht, ob
  ich meinen Bruder mag} (\textit{I don't know whether I like my
  brother}).  In this example, neither we nor the author actually know
...
disagreement with positive evaluations and, consequently, act as
negative judgments.  Special care should be taken when dealing with
questions and subjunctive sentences (see FAQ Section in the extended
...
expression (typically a polar-term).  After determining these two
elements, you shall put the \texttt{sentiment} tags around the
\emph{minimal complete syntactic or discourse-level unit in which both
  (target and evaluation expression) appear together}.
...
\begin{center}
  \begin{table}[htb!]
    \begin{tabular}{l|c|p{0.957\clmnwidth}}\toprule
      \textbf{Attribute} & \textbf{Value} & \textbf{Meaning}\\\midrule
      %%%%%%%%%%%%%%%%%%%

      & \textit{positive} & sentiment expresses a positive attitude to
      its respective target, \eg{} \textit{Es war ein fantastischer
        Abend (It was a fantastic evening)};\\\cmidrule{2-3}

      & \textit{negative\newline(default)} & sentiment expresses a
...
evaluated by a sentiment.

Because sentiments are required to be evaluative, there must always be
...
the object of evaluation, so you shall label it as a \texttt{target}
element.
...
object that is dispreferred in the comparison (see Example
\ref{exmp:trg-comp}).
\begin{example}
...
\begin{center}
  \begin{table}[htb!]
    \begin{tabular}{l|c|p{0.895\clmnwidth}}\toprule
      \textbf{Attribute} & \textbf{Value} & \textbf{Meaning}\\\midrule

      & \textit{true} (default) & in comparisons, this value means
      that the respective target is considered better than another
      compared object, \eg{} \textit{\emph{Die neue Frisur} passt ihr
        garantiert besser als die alte (\emph{The new hairstyle} suits
        her definitely better than the old one)};\\\cmidrule{2-3}
...
\paragraph{Definition.} Sentiment \emph{sources} are immediate author(s)
or holder(s) of evaluative opinions.  These are typically the author
...
annotate the citing person as a \texttt{source} (see Example
...
\begin{center}
  \begin{table}[htb!]
    \begin{tabular}{l|c|p{0.895\clmnwidth}}\toprule
      ...
      sentiment-ref & \textit{$\longrightarrow$\newline(directed
        edge)} & see Table \ref{tbl:target}\\\hline
...
\begin{itemize}
  ...
  \item smileys, \eg{} :), :-(, \smiley{}, \frownie{}.
\end{itemize}
...
\begin{center}
  \begin{table}[htb!]
    \begin{tabular}{l|c|p{0.895\clmnwidth}}\toprule
      ...
      & \textit{positive} & polar term has a positive evaluative
      ...
      \multirow{2}{*}{sarcasm} & \textit{true} & polar-term is
      derisive, \ie{} its actual polarity is the opposite of its
...
\begin{center}
  \begin{table}[htb]
    \begin{tabular}{l|c|p{0.895\clmnwidth}}\toprule
      ...
      strongly increases the polar sense and stylistic markedness of
      the polar term, \eg{} \textit{sehr (very), super (super), stark
        (strongly)} etc.\\\hline

      %%%%%%

      polar-term-ref & \textit{$\longrightarrow$\newline(directed
...
\begin{center}
  \begin{table}[hb]
    ...
    \end{tabular}
    \caption{Attributes of \texttt{diminisher}s}
...
\begin{center}
  \begin{table}
    \begin{tabular}{l|c|p{0.875\clmnwidth}}\toprule
      \textbf{Attribute} & \textbf{Value} & \textbf{Meaning}\\\midrule
      polar-term-ref & \textit{$\longrightarrow$\newline(directed
        edge)} & an edge from \texttt{negation} to the
      \texttt{polar-term} being negated\\\bottomrule
    \end{tabular}
    \caption{Attributes of \texttt{negation}s}
    \label{tbl:negation}
  \end{table}
\end{center}
...
labeled \textit{Jubel von \#CDU,... \"uber den Tod von ...}
(\textit{the exultation of the \#CDU ... about the death of ...}) as
\texttt{sentiment}, because the truth status of this statement is
unknown.  But, on the other hand, the mere hypothesis that a political
party could experience a glee feeling because of a renter's death is
sarcastic.  We can recognize it from the \texttt{polar-term}
\textit{\#Jubel} (\textit{\#exultation}), whose prior semantic
orientation is positive, but which suggests a negative attitude to the
CSU party in this context, without any explicit contextual modifiers.
Accordingly, we set the (prior) \texttt{polarity} of the term to
\texttt{positive}, the polarity of its \texttt{sentiment} to
\texttt{negative}, and the \texttt{sarcasm} attribute of both labels
to \texttt{true}.  Apart from having different polarities,
\texttt{sentiment} and \texttt{polar-term} also have different
intensities: since \textit{\#Jubel} (\textit{\#exultation}) expresses
a higher degree of excitement than the word \textit{Freude}
(\textit{joy}), we set its \texttt{intensity} to \texttt{high}.  On
the other hand, the overall sentiment expression is rather subtle and
does not show high exaggeration of the author.  So, we set the
\texttt{intensity} of the \texttt{sentiment} to \texttt{medium} rather
than \texttt{high}.
