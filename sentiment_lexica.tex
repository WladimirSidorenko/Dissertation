...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \EndFunction
    \Statex
    \Function{FindMeanAxis}{$\vars{M}, \mathcal{S}_1, \mathcal{S}_2$}
    ...
    \State\vars{O}$_{subj}\gets\frac{\mu_{\mathcal{S}} -
    ...
    \State\vars{max\_score}$_{subj}\gets\max\left(\{|\vars{n}\lbrack\vars{u}_{subj}\rbrack%
      - \vars{O}_{subj}| |\forall\vars{n}\in\func{cols}(\vars{M})\}\right)$;

    \State\vars{O}$_{pol}\gets\frac{\mu_{\mathcal{P}} -
...
gradient, we can trivially optimize the vector~$\vec{b}$ using
gradient ascent until we reach the global maximum of the function~$f$.
...
\todo[inline]{Add description of the two-stage procedure.}
Eventually, we created the final list of polar terms, sorting
