...
The value of the feature function $f$ at position $m$ is then normally
defined as:
\begin{equation}
  ...
  \end{cases}
\end{equation}
The most common predicate used for this type of the attributes is a
simple unit function~$\phi$, which always yields one regardless of the
data input $\mathbf{x}$.  However other, more complex conditions are
...
the input data $\mathbf{x}$ at position $m$ and the label $y'$ at the
respective state:
\begin{equation}
  ...
  \end{cases}
\end{equation}
This type of the attributes commonly accounts for the overwhelming
...
\item\emph{formal}, which included the initial three characters of
  ...
  }\mathbf{x}_m[:3] == \mbox{"abc" else } 0$), its last three
  characters, and the general spelling class (e.g., alphanumeric,
  digit, or punctuation);

\item\emph{morphological}, which encompassed the PoS tag of the token,
  case and gender values for inflectable PoS types, degree of
  comparison for adjectives; and mood, tense, and person forms for
  verbs;
...
\item and, finally, \emph{syntactic}, which included the dependency
  ...
  parent, the lemma of its parent node as a unigram feature, the PoS
  tag and the polarity class of the grandparent, and the overall
  polarity class of all children.  The latter class was computed by
  summing up the polarity scores of all immediate descendants and
  checking whether this result was greater, less, or equal to zero.
\end{itemize}

In addition to the above attributes, we also used a set of complex
lexical-syntactic features which combined several semantic and
syntactic traits into a single function.  These complex features
included the dependency relation of the previous token in the sentence
to its parent + dependency relation of the current token to its
ancestor; the dependency relation of the next word in the sentence +
dependency relation of the current token; the lemma of the child node
+ dependency relation of this child; the PoS tag of the child node +
its dependency relation + the pos-tag of the current token.
