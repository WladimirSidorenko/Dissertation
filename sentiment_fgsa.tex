...
defined features outperform both recurrent deep learning systems (LSTM
and GRU), reaching a macro-averaged \F-score of~0.287 on three
opinion-relevant entity classes (sentiments, targets, and sources).  A
closer look at each of the settings in these experiments revealed that:
\begin{itemize}
\item Higher sentiment scores can be achieved by narrowing the spans
  ...
  might negatively affect the results of other opinion entities
  (targets and sources);
\item Even though the context, redefining the topologies of both PGM and RNN
\item 
\item 
\end{itemize}
