...
pre-selected by heuristic rules), reaching an \F-score of 68\%
measured with the head-matching metric.
...
The allegedly first attempt to use semi-Markov CRFs in an opinion
mining application was made by \citet{Yang:12} who first determined
the boundaries of potential DSE (\emph{direct speech events}) and ESE
(\emph{expressive subjective elements}) based on a shallow syntastic
parsing and then let a semi-Markov model determine the most likely
label assignment for these segments.  The authors reached remarkable
67.17\% for DSEs and 54.47\% for ESEs measured with proportional
overlap metric, surpassing the baselines for these tasks established
by \citet{Breck:07} and \citet{Johansson:10,Johansson:11}
