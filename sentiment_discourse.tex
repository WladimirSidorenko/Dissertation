...
structural hierarchy, even units of the same language level might play
a different role and be of varying importance while joined
...
comprise the gist of the complete text.
...
discourse information to the PotTS and SB10k data.  Then, in
...
(hierarchical) coherence relations between these EDUs.
...
Finally, \citet{Winter:77} presented an extensive study of different
lexical means, which could connect two sentences, and also devised a
taxonomy of semantic relattions based on these observations.  All
links in this taxonomy were divided into two main groups:
\textsc{Matching} and \textsc{Logical Sequence}; depending on whether
a sentence was elaborating on the preceding content of the text or
rather adding new information to the narrative.
...
could be easily inferred from context.  The authors denoted these
cases as \textsc{Implicit} discourse relations and asked the
annotators of the treebank to annotated the arguments of such
relations as well, but, in addition, also specify the connective word
or phrase, which, in their opinion, was omitted.
...
the SDRT analysis of the same WSJ fragment in
...
theory: for example, the \textsc{Source} relation in SDRT almost
completely corresponds to the \textsc{Attribution} edge in RST.  On
the other hand, SDRT's \textsc{Contrast} subsumes RST's
...
relationship between the arguments.  These discrepancies between
...
\ref{dasa:fig:sdrt-graph}.  Another dissimilarity stems from the
different scopes of the commentary \texttt{While that's not
  spectacular}: while the SDRT graph suggests that this opinion
primarily relates to the actual statement of Neil Sweig, RST tree
widens the modification scope of this opinion also to the fact of
...
English that we could choose from
\cite[\eg][]{Hernault:10,Feng:14,Ji:14,Yoshida:14,Joty:15}.

\section{Data Preparation}\label{sec:dasa:data}

To prepare data for our experiments, we split all microblogs from the
PotTS and SB10k corpora into elementary discourse units using the
...
surprising to see that many segments in a single message, we decided
to look at these tweets in detail.  As it turned out, such high number
of EDUs typically arose from spurious punctuation marks, which were
carelessly used in texted messages, but evidently confused our
...
N\"achte hilft die beste Kur nicht.''  [\emph{Even the best cure won't
    help against such nights.}]) lower than the third one (``Aber
Kaffee!''  [\emph{But coffee!}]), although traditional RST would
consider both units as equally important and use the multi-nuclear
