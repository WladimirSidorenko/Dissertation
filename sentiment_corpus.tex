...
\item
  ...
  satisfy in order to be considered as a sentiment in our corpus were:
  ...
  \item \textit{subjectivity}, i.e., the expressed opinion had to be a
    personal attitude, not verifiable by any objective means; and,
    finally,
  \item an \textit{evaluative} nature, i.e., the judgment had to refer
    to some clearly discernable target from its surrounding context.
  \end{inparaenum}
  ...
  attribute.  Further attributes associated with the sentiment
  elements included \emph{intensity} with the possible values ranging
  over weak, medium, and strong; and the boolean attribute
  \emph{sarcasm}, with which our annotators marked opinions whose
  actual polarity was the opposite of their apparent form, e.g.,
  ``Mein j\"ungerer Bruder ist in der Pr\"ufung durchgefallen.
  Klasse!''  (\textit{My younger brother has failed his exam.  Well
    done!});

\item
  another annotation element closely related to sentiments were the
  \textbf{targets}, which we defined as objects or events being
  evaluated by sentiment expressions.  For this element, we introduced
  two types of attributes: the boolean feature \emph{preferred} and
  the link attributes \emph{anaphref} and \emph{sentiment-ref}.  The
  first feature was used for comparative opinions to distinguish
  targets which were given preference in the given comparison from the
  dispreferred objects.  The \emph{anaphref} attribute was used for
  elements expressed by pronouns and pointed to the respective
  antecedents of these proforms (cf. \citet{Stoyanov:06} and
  \citet{Ding:10} on the importance of coreference resolution in
  opinion mining).  Finally, the \emph{sentiment-ref} edges were
  pointing from a target to its respective sentiment in the cases when
  the target was locate outside of the corresponding opinion or vice
  versa on the intersection of two sentiments;

\item
  the second crucial component of sentiments next to targets were the
  \textbf{sources} of the opinions, which denoted immediate author(s)
  or holder(s) of evaluative judgments.  The only attribute associated
