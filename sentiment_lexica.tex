...
we will evaluate the aforementioned sentiment resources on the updated
data in order to obtain a baseline for our subsequent experiments.
Since literally all the current GSL were created using an automatic
translation of English opinion lexica with a manual post-editing of
the translated entries, we will then investigate whether the original
methods that were used initially for creating the English sources
would yield better results when applied to German data directly.
Finally, in the concluding step, we will analyze whether one of most
popular areas of research in contemporary CL---distributed vector
...
data.  We will summarize and draw conclusions from our experiments in
the last part of this section, also discussing which of the presented
methods performed best on our corpus.
