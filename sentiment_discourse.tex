...
classifying the overall polarity of messages, and DL-based approaches
...
considering it as a single whole (for example, of-features
classifiers) or analyzing it as a monotone sequence of equally
important elements (for instance, recurrent neural networks).
Unfortunately, both of these solutions completely ignore the
hierarchical nature of language \cite{Saussure:13,Hjelmslev:70}, in
which morphemes are put together to form words, words are united
together into sentences, and multiple sentences constitute a
discourse.  Moreover, even units of the same linguistic level might
play different roles and be of different importance while joined
syntagmatically: \eg{} in words, the root morpheme typically conveys
more lexical meaning than the affixes; in sentences, the syntactic
head grammatically dominates all its dependents.  The same kind of
hierarchy can also be obsered in discourse, where one of the sentences
might express the gist of the whole text, but its meaning can still be
changed or reversed by other propositions.
...
level of words to the level of the whole message), we decided to check
whether making the last of these solutions (our LBA method) aware of
the discourse relations would improve its results.  However, before we
proceed with our experiments, we will first give a short overview of
the most popular approaches to discourse analysis in the literature
...
\todo[inline]{General Discourse Theory}

\citet{Hobbs:79}

\todo[inline]{RST: \citet{Mann:88}}

\citet{Thompson:87}

\citet{Mann:88}

\textbf{RST} One of the first comprehensive theory of discourse that
was directly amenable to automatic analysis was proposed
by~\citet{Mann:88}.  In their approach, called \emph{Rhetorical
  Structure Theory} or \emph{RST}, the authors suggested to divide the text into

A relation definition consists of four fields:
1. Constraint on the Nucleus;
2. Constraint on the Satellite;
